 ...
 * <p>1. 支持链式 bean
 *
 * <p>2. 自定义的 BeanCopier 解决 spring boot 和 cglib ClassLoader classLoader 不一致的问题
 *
 * @author: chow
...
public abstract class BaseBeanCopier {
  ...
  public static BaseBeanCopier create(
      Class<?> source, Class<?> target, ClassLoader classLoader, boolean useConverter) {
    Generator gen;
    if (classLoader == null) {
      gen = new Generator();
    } else {
