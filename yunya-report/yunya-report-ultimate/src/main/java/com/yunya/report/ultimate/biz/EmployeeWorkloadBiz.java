 ...
 * 简介：员工工作量服务层
 *
 * @author: chenlin @Description: 员工工作量服务层 @Date: 2021/10/25 14:29
 * @since: 1.0.0
 */
@Service
public class EmployeeWorkloadBiz {
   ...
   * @return
   */
  public PageInfo<ClinicEmployeeWorkloadOfOperationVO> findEmployeeWorkloadListOfOperation(
   ...
   * @return
   */
  private List<ClinicEmployeeWorkloadOfOperationVO> findEmployeeWorkloadList(
      ClinicEmployeeWorkloadQuery query) throws Exception {
    // 主数据：门诊 + 员工
    ...
    Future<Map<String, BigDecimal>> freePaymentWorkload =
        findClinicEmployeeFreePaymentWorkload(query);
    ...
    // 数据组装
    List<ClinicEmployeeWorkloadOfOperationVO> result =
        mergeEmployeeWorkload(
            employee,
            receivableWorkload,
            receivedWorkload,
            supplementWorkload,
   ...
   * @return
   */
  private List<ClinicEmployeeWorkloadOfOperationVO> mergeEmployeeWorkload(
   ...
   * @return
   */
  private Future<Map<String, EmployeeWorkloadCost>> findClinicEmployeeWorkCost(
      ClinicEmployeeWorkloadQuery query) {
    return threadPool.submit(
        () -> {
          List<EmployeeWorkloadCost> fee =
              employeeWorkloadCostBiz.findClinicEmployeeWorkCost(query);
   ...
   * @return
   */
  private Future<Map<String, BigDecimal>> findClinicEmployeeRefundWorkload(
      ClinicEmployeeWorkloadQuery query) {
    return threadPool.submit(
        () -> {
          List<EmployeeWorkloadVO> workloads =
              baseBillDetailMapper.selectClinicEmployeeRefundWorkload(query);
   ...
   * @return
   */
  private Future<Map<String, BigDecimal>> findClinicEmployeeFreePaymentWorkload(
      ClinicEmployeeWorkloadQuery query) {
    return threadPool.submit(
        () -> {
          List<EmployeeWorkloadVO> workloads =
              baseBillDetailMapper.selectClinicEmployeeFreePaymentWorkload(query);
          return mapEmployeeWorkload(workloads);
        });
  }
   ...
   * @return
   */
  public Future<Map<String, BigDecimal>> findClinicEmployeeReceivedWorkload(
      ClinicEmployeeWorkloadQuery query, boolean groupByOrgId) {
    return threadPool.submit(
        () -> {
          List<EmployeeWorkloadVO> workloads =
              baseBillDetailMapper.selectClinicEmployeeReceivedWorkload(query, groupByOrgId);
          return mapEmployeeWorkload(workloads);
        });
  }
   ...
   * @return
   */
  private Future<Map<String, BigDecimal>> findClinicEmployeeReceivableWorkload(
      ClinicEmployeeWorkloadQuery query) {
    return threadPool.submit(
        () -> {
          List<EmployeeWorkloadVO> workloads =
   ...
   * @return
   */
  private Map<String, EmployeeTariffWorkloadVO> mapEmployeeWorkloadVO(
      List<EmployeeTariffWorkloadVO> workloads) {
    Map<String, EmployeeTariffWorkloadVO> result = new HashMap<>();
    workloads.forEach(
        vo -> result.put(vo.getEmployeeId() + "," + vo.getOrgId() + "." + vo.getItemId(), vo));
    return result;
  }
     ...
     * @return
     */
    private Map<String, EmployeeTariffWorkloadVO> allExportmapEmployeeWorkloadVO(
            List<EmployeeTariffWorkloadVO> workloads) {
        Map<String, EmployeeTariffWorkloadVO> result = new HashMap<>();
        workloads.forEach(
                vo -> result.put(vo.getEmployeeId() + "," + vo.getOrgId() + "." + vo.getItemId()+ "." + vo.getBillId(), vo));
        return result;
    }
   ...
   * @throws Exception
   */
  public PageInfo<ClinicEmployeeWorkloadOfPersonnelVO> findEmployeeWorkloadListOfPersonnel(
      ClinicEmployeeWorkloadQuery query) throws Exception {
  ...
  /**
   * 计算奖金： 应收奖金、应收奖金基数 实收奖金、实收奖金基数 转换实体：ClinicEmployeeWorkloadOfOperationVO ->
   * ClinicEmployeeWorkloadOfPersonnelVO
   *
   * @param list
   */
  private List<ClinicEmployeeWorkloadOfPersonnelVO> computeBonusAndConvertPersonnel(
      List<ClinicEmployeeWorkloadOfOperationVO> list) {
   ...
   * @return
   */
  private ClinicEmployeeWorkloadOfPersonnelVO convertEntity(
      ClinicEmployeeWorkloadOfOperationVO vo) {
  ...
  public void exportEmployeeWorkloadListOfOperation(
    ...
    List<ClinicEmployeeWorkloadOfOperationVO> resultList =
  ...
  public void exportEmployeeWorkloadListOfPersonnel(
      HttpServletResponse response, ClinicEmployeeWorkloadQuery query) throws Exception {
    query.setWhetherPage(false);
    List<ClinicEmployeeWorkloadOfPersonnelVO> resultList =
        findEmployeeWorkloadListOfPersonnel(query).getList();
...
//        findClinicExecutorTariffFreePaymentAmount(query);
    ...
    Future<Map<String, EmployeeTariffWorkloadVO>> supplementWorkload =
        findClinicExecutorTariffFreeSupplementWorkload(query);
...
//                allExportfindClinicExecutorTariffFreePaymentAmount(query);
        ...
        // 补入工作量
        Future<Map<String, EmployeeTariffWorkloadVO>> supplementWorkload =
                allExportfindClinicExecutorTariffFreeSupplementWorkload(query);

        // 退费工作量
        Future<Map<String, EmployeeTariffWorkloadVO>> refundWorkload =
                allExportfindClinicExecutorTariffRefundWorkload(query);
...
//                        freePaymentAmount,
      ...
      String fileName = query.getStartDate() + "-" + query.getEndDate() + "收费数量及金额全部明细";
      ExcelUtil<BillItemTollAndWorkloadAllExportVO> excelUtil =
              new ExcelUtil<>(BillItemTollAndWorkloadAllExportVO.class);
   ...
   * @return
   */
  private BillItemTollAndWorkloadVO createWorkloadBaseInfo(
      String key,
     ...
     * @return
     */
    private BillItemTollAndWorkloadAllExportVO allExportcreateWorkloadBaseInfo(
            String key,
            Map<String, ClinicEmployeBonusCoefficientVO> employeeMap,
            Map<Integer, ItemCategoryVO> tariffMap) {
        String[] keys = key.split("\\.");
        BillItemTollAndWorkloadAllExportVO entity = new BillItemTollAndWorkloadAllExportVO();
        ClinicEmployeBonusCoefficientVO employee = employeeMap.get(keys[0]);
        if (!ObjectUtils.isEmpty(employee)) {
            entity.setAbbreviation(employee.getAbbreviation());
            entity.setOrgId(employee.getOrgId());
   ...
   * @return
   */
  private Future<Map<String, EmployeeTariffWorkloadVO>> findClinicExecutorTariffRefundWorkload(
      BillItemTollWorkloadQuery query) {
    return threadPool.submit(
        () -> {
          List<EmployeeTariffWorkloadVO> workload =
              baseBillDetailMapper.selectClinicExecutorTariffRefundWorkload(query);
     ...
     * @return
     */
    private Future<Map<String, EmployeeTariffWorkloadVO>> allExportfindClinicExecutorTariffRefundWorkload(
            BillItemTollWorkloadQuery query) {
        return threadPool.submit(
                () -> {
                    List<EmployeeTariffWorkloadVO> workload =
   ...
   * @return
   */
  private Future<Map<String, EmployeeTariffWorkloadVO>>
      findClinicExecutorTariffFreeSupplementWorkload(BillItemTollWorkloadQuery query) {
    return threadPool.submit(
        () -> {
          List<EmployeeTariffWorkloadVO> workload =
              baseBillDetailMapper.selectClinicExecutorTariffSupplementWorkload(query);
          return mapEmployeeWorkloadVO(workload);
        });
  }
    /**
     ...
     */
    private Future<Map<String, EmployeeTariffWorkloadVO>>
    allExportfindClinicExecutorTariffFreeSupplementWorkload(BillItemTollWorkloadQuery query) {
        return threadPool.submit(
                () -> {
                    List<EmployeeTariffWorkloadVO> workload =
   ...
   * @return
   */
  private Future<Map<String, EmployeeTariffWorkloadVO>> findClinicExecutorTariffFreePaymentAmount(
      BillItemTollWorkloadQuery query) {
    return threadPool.submit(
        () -> {
          List<EmployeeTariffWorkloadVO> amount =
              baseBillDetailMapper.selectClinicExecutorTariffFreepaymentAmount(query);
     ...
     * @return
     */
    private Future<Map<String, EmployeeTariffWorkloadVO>> allExportfindClinicExecutorTariffFreePaymentAmount(
            BillItemTollWorkloadQuery query) {
        return threadPool.submit(
                () -> {
                    List<EmployeeTariffWorkloadVO> amount =
                            baseBillDetailMapper.allExportfindClinicExecutorTariffFreePaymentAmount(query);
                    return allExportmapEmployeeWorkloadVO(amount);
                });
    }
  /**
   * 多线程查询执行人的项目实收工作量
   ...
   * @return
   */
  public Future<Map<String, EmployeeTariffWorkloadVO>> findClinicExecutorTariffReceivedWorkload(
      BillItemTollWorkloadQuery query) {
    return threadPool.submit(
        () -> {
          List<EmployeeTariffWorkloadVO> workload =
     ...
     * @return
     */
    public Future<Map<String, EmployeeTariffWorkloadVO>> allExportfindClinicExecutorTariffReceivedWorkload(
            BillItemTollWorkloadQuery query) {
        return threadPool.submit(
                () -> {
                    List<EmployeeTariffWorkloadVO> workload =
  ...
  private Comparator itemWorkloadCmpList() {
    ...
    private Comparator allExportitemWorkloadCmpList() {
        return SortUtil.comparing(
                BillItemTollAndWorkloadAllExportVO::getReceivedWorkload,
                BillItemTollAndWorkloadAllExportVO::getFreePayWorkload,
                BillItemTollAndWorkloadAllExportVO::getSupplyWorkload,
                BillItemTollAndWorkloadAllExportVO::getRefundWorkload)
                .reversed();
    }
   ...
   * @return
   */
  public PageInfo<ClinicEmployeeWorkloadOfOperationVO> findWorkloadListOfConsulter(
      ClinicEmployeeWorkloadQuery query) throws Exception {
    List<ClinicEmployeeWorkloadOfOperationVO> result = findEmployeeWorkloadList(query);
    // 分页
    return PageUtl.doPage(query.getPageNum(), query.getPageSize(), result, query.getWhetherPage());
