...
public interface BaseBillMapper extends Mapper<BaseBill> {
   ...
   * @return 订单号集合
   */
  List<BillIdVo> findBillIdList(
      @Param("query") PatientOriginEmployeeQuery query,
      @Param("list") List<PatientOriginEmployeeVo> patientOriginEmployeeVoList);
   ...
   * @return 已收工作量信息
   */
  List<Integer> findBaseBillIdList(
   ...
   * @return 补入工作量明细
   */
  List<ReceivedWorkloadDetailsVo> selectMakeUpDetail(
      @Param("query") ReceiverkLoadQuery query, @Param("originType") Integer originType);
