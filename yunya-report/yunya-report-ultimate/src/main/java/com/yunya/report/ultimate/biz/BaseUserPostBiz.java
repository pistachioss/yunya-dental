...
public class BaseUserPostBiz extends BaseBiz<BaseUserPostMapper, BaseUserPost> {
   ...
   * @return
   */
  private List<AssistantMatchingStatisticsVO> getAssistant2List(EmployeeMatchingRecordQuery query, CountDownLatch latch) throws ExecutionException, InterruptedException {
    List<AssistantMatchingStatisticsVO> result;
    try{
      Future<List<AssistantMatchingStatisticsVO>> assistant_2 = executorService.submit(() -> mapper.selectAssistantMatchingStatisticsByAssistant("assistant_2", query));
   ...
   * @return
   */
  private List<AssistantMatchingStatisticsVO> getAssistant3List(EmployeeMatchingRecordQuery query, CountDownLatch latch) throws ExecutionException, InterruptedException {
    List<AssistantMatchingStatisticsVO> result;
    try{
      Future<List<AssistantMatchingStatisticsVO>> assistant_3 = executorService.submit(() -> mapper.selectAssistantMatchingStatisticsByAssistant("assistant_3", query));
   ...
   */
  private List<AssistantMatchingStatisticsVO> getFutureObj(Future<List<AssistantMatchingStatisticsVO>> futureList) throws ExecutionException, InterruptedException {
      List<AssistantMatchingStatisticsVO> assistantMatchingStatisticsVOS = futureList.get();
    return assistantMatchingStatisticsVOS == null ? new ArrayList<>() : assistantMatchingStatisticsVOS;
  }
  ...
  public void exportEmployeeMatchingStatisticsList(
      HttpServletResponse response, EmployeeMatchingRecordQuery query) throws IOException, InterruptedException, ExecutionException {
