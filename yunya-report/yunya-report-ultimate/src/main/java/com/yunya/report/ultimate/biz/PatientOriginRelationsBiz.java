...
public class PatientOriginRelationsBiz extends BaseBiz<BasePatientOriginLogMapper, BasePatientOriginLog> {
     ...
     * @return 推荐人推荐信息
     */
    private List<PatientOriginEmployeeVo> combinationEmployeeReferral(PatientOriginEmployeeQuery query) {
        // 查询推荐人信息以及推荐数量
     ...
     * @param query 条件
     */
    private void getMakeUpWorkload(List<PatientOriginEmployeeVo> patientOriginEmployeeVoList, PatientOriginEmployeeQuery query) {
        for (PatientOriginEmployeeVo patientOriginEmployee : patientOriginEmployeeVoList ) {
            BigDecimal makeUpWorkload = baseBillDetailMapper.findMakeUpWorkload(patientOriginEmployee.getOriginId(),query);
     ...
     * @param query 条件
     */
    private void getTotalRefundAmount(List<PatientOriginEmployeeVo> patientOriginEmployeeVoList, PatientOriginEmployeeQuery query) {
        if (patientOriginEmployeeVoList != null){
            for (PatientOriginEmployeeVo patientOriginEmployeeVo:  patientOriginEmployeeVoList) {
                BigDecimal totalRefundAmount = baseRefundMapper.findRefundAmount(patientOriginEmployeeVo.getOriginId(),query);
     ...
     * @param query 条件
     */
    private void getReceivedTotalWorkload(List<PatientOriginEmployeeVo> patientOriginEmployeeVoList, PatientOriginEmployeeQuery query, Boolean type){
        // 获取全部订单记录Map
        ...
        if (patientOriginEmployeeVoList.size() > 0){
            for (PatientOriginEmployeeVo patientOriginEmployeeVo : patientOriginEmployeeVoList ) {
                ...
                List<ReceivedTotalWorkloadVo> receivedTotalWorkloadVoList = mapper.findReceivedTotalWorkload(patientOriginEmployeeVo);
     ...
     */
    private Map<Integer, List<BaseBillPay>> getBaseBillPayInfoMap(PatientOriginEmployeeQuery query,Boolean type) {
     ...
     * @param query 查询条件
     */
    public void exportEmployeeReferralList(HttpServletResponse response, PatientOriginEmployeeQuery query) throws IOException {
        if (StringHelper.isNotEmpty(query.getEndDate())) {
            String endDate = new DateTime(query.getEndDate()).plusDays(1).toString("yyyy-MM-dd");
            query.setEndDate(endDate);
        }
    ...
    public List<ReceivedWorkloadDetailsVo> finlereceiverkLoad(ReceiverkLoadQuery query) {
        ...
        // 订单id List
        List<Integer> baseBillIdList = baseBillMapper.findBaseBillIdList(query);
        if (baseBillIdList != null){
            // 订单支付明细
            List<BaseBillPay> baseBillPayList = baseBillPayMapper.selectBaseBillPayInfoList(baseBillIdList,query);
                       ...
                       if (baseBillPayList != null){
                           for (BaseBillPay baseBillPay : baseBillPayList) {
                               ...
                               if (receivedWorkloadDetailsVo.getBillId().equals(baseBillPay.getBillId())){
                                   ...
                                   receivedWorkloadDetailsVo.setWorkload(multiply.setScale(2,BigDecimal.ROUND_DOWN));
                                   // 加入到结果返回集合中
