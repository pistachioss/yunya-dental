...
public class PatientOriginRelationsBiz
    ...
    if (patientOriginEmployeeVoList != null) {
      for (PatientOriginEmployeeVo patientOriginEmployeeVo : patientOriginEmployeeVoList) {
        BigDecimal totalRefundAmount =
            baseRefundMapper.findRefundAmount(
                patientOriginEmployeeVo.getOriginId(), query.getStartDate(), query.getEndDate(), 1);
  ...
  private void getReceivedTotalWorkload(
      ...
      PatientOriginEmployeeQuery query,
      Boolean type,
      List<BillIdVo> BillIdVoList) {
    // 获取全部订单记录Map
    Map<Integer, List<BaseBillPay>> baseBillPayInfoMap =
        getBaseBillPayInfoMap(query, type, BillIdVoList);
      ...
      for (PatientOriginEmployeeVo patientOriginEmployeeVo : patientOriginEmployeeVoList) {
        ...
        if (billIds != null && billIds.size() > 0) {
          ...
          List<ReceivedTotalWorkloadVo> receivedTotalWorkloadVoList =
              mapper.findReceivedTotalWorkload(billIds);
                ...
                for (BaseBillPay baseBillPay : baseBillPays) {
                  ...
                  if (type) {
                    patientOriginEmployeeVo.setReceivedTotalWorkload(
                        patientOriginEmployeeVo
                            .getReceivedTotalWorkload()
                            .add(multiply.setScale(2, BigDecimal.ROUND_HALF_UP)));
                  } else {
                    patientOriginEmployeeVo.setFreeTotalWorkload(
                        patientOriginEmployeeVo
                            .getFreeTotalWorkload()
                            .add(multiply.setScale(2, BigDecimal.ROUND_HALF_UP)));
                  }
                }
              }
  ...
  private Map<Integer, List<BaseBillPay>> getBaseBillPayInfoMap(
      PatientOriginEmployeeQuery query, Boolean type, List<BillIdVo> billIdList) {
    ...
    if (type) {
      ...
      baseBillPayList =
          baseBillPayMapper.findMdBaseBillPayInfoList(
              billIdList, query.getStartDate(), query.getEndDate(), itemIds);
    }
    // k 订单id v 订单记录
   ...
   * @return 补入工作量明细
   */
  private List<ReceivedWorkloadDetailsVo> makeUpDetail(ReceiverkLoadQuery query, Integer originType)
      throws ParseException {
    ...
    if (receivedWorkloadDetailsVoList != null) {
      for (ReceivedWorkloadDetailsVo receivedWorkloadDetailsVo : receivedWorkloadDetailsVoList) {
        // 判断关联时间是否大于初诊时间
        receivedWorkloadDetailsVo.setIsChange(
            activityRelationsBiz.getIsChange(receivedWorkloadDetailsVo));
      }
   ...
   * @return 退费金额明细
   */
  private List<ReceivedWorkloadDetailsVo> refundDetail(ReceiverkLoadQuery query, Integer originType)
      throws ParseException {
    ...
    if (refundIdList != null) {
      for (Integer refundId : refundIdList) {
        ...
        if (receivedWorkloadDetailsVoList != null) {
          for (ReceivedWorkloadDetailsVo receivedWorkloadDetailsVo :
              receivedWorkloadDetailsVoList) {
            // 判断关联时间是否大于初诊时间
            receivedWorkloadDetailsVo.setIsChange(
                activityRelationsBiz.getIsChange(receivedWorkloadDetailsVo));
          }
  ...
  public List<ReceivedWorkloadDetailsVo> receivedDetail(
    ...
    if (baseBillIdList != null) {
      ...
      Map<Integer, List<BaseBillPay>> baseBillPayMap =
          getBaseBillPayList(baseBillIdList, baseBillPayList);
      // 订单支付明细
        ...
        if (baseBillDetailBizVos != null) {
          for (ReceivedWorkloadDetailsVo receivedWorkloadDetailsVo : baseBillDetailBizVos) {
            ...
            receivedWorkloadDetailsVo.setIsChange(
                activityRelationsBiz.getIsChange(receivedWorkloadDetailsVo));
            // 通过项目订单id 获取订单支付记录
            List<BaseBillPay> billPayList =
                baseBillPayMap.get(receivedWorkloadDetailsVo.getBillId());
   ...
   */
  public Map<Integer, List<BaseBillPay>> getBaseBillPayList(
      List<Integer> baseBillIdList, List<BaseBillPay> baseBillPayList) {
