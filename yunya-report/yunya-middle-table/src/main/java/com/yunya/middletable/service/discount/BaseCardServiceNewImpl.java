...
@Slf4j
@Service
public class BaseCardServiceNewImpl {
    ...
    public void pullCard(String startDateStr, String endDateStr) throws ExecutionException, InterruptedException {
    ...
    private List<BaseCard> getOriginData(List<Card> originCards) {
        ...
        Map<Integer, String> saleChannelMap = this.getSaleChannelMap();
        return originCards.stream().map(card -> singleEntityTransform(card, accountMap, allocates, deadlineBo, saleChannelMap))
                .collect(toList());
    }
    ...
    private BaseCard singleEntityTransform(Card card, Map<Integer, String> accountMap, List<CouponAllocate> allocates,
                                           List<BaseCardBo> deadlineBo, Map<Integer, String> saleChannelMap) {
    ...
    public Map<Integer, String> getSaleChannelMap() {
        Example example = new Example(SalesChannel.class);
        example.selectProperties("id", "name");
        return salesChannelMapper.selectAll()
                .stream().collect(toMap(SalesChannel::getId, SalesChannel::getName));
    }
