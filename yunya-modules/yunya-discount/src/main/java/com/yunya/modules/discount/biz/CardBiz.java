...
public class CardBiz extends BaseBiz<CardMapper, Card> {
  ...
  @Resource private CouponCommonInfoMapper couponMapper;
  @Resource private CouponAllocateMapper allocateMapper;
  @Resource private VoucheCouponMapper voucherMapper;
  @Resource private VoucherDiscountItemMapper voucherDiscountItemMapper;
  @Resource private DiscountCouponMapper discountMapper;
  @Resource private PackageCouponMapper packageMapper;
  @Resource private PackageCouponItemMapper packageCouponItemMapper;
  @Resource private SpecialPackageCouponItemMapper specialPackageCouponItemMapper;
  @Resource private SpecialPackageCouponMapper specialPackageMapper;
  @Resource private SalesChannelMapper salesChannelMapper;
  @Resource private ProductTypeMapper productTypeMapper;
  @Resource private RechargeCardMapper rechargeCardMapper;
  @Resource private CardCancelLogMapper cardCancelLogMapper;
  ...
  @Resource private CardBenefitMapper cardBenefitMapper;
  ...
  // 优惠券是否共用
  private static final ThreadLocal<Map<Integer, List<Integer>>> mixUsedThreadLocal =
  ...
  public PageInfo<GenerateAllocatePageVo> getCouponAllocatePage(CouponAllocateQuery query) {
    Page<GenerateAllocatePageBo> page =
        PageHelper.startPage(query.getPageNum(), query.getPageSize());
    // 分页查询
    couponMapper.listBatchAllocateByParam(query.getCouponName(), query.getCouponTypeList());
    // 属性转换
    List<GenerateAllocatePageVo> list =
        page.getResult().stream().map(this::allocateBoConvertVo).collect(toList());
  ...
  public List<GenerateAllocateDetailVo> getGenerateAllocateList(GenerateAllocateDetailQuery query) {
    ...
    example
        .createCriteria()
        .andEqualTo("couponId", query.getCouponId())
  ...
  public ResponseResult generateAllocate(GenerateAllocateModel allocateModel) throws Exception {
    ...
    String lockKey =
        Joiner.on(":")
            .join(
                RedisConstants.LOCK_CARD_GENERATE,
                String.valueOf(couponId),
                submitDate.toEpochSecond(ZoneOffset.of("+8")));
    ...
    try {
      ...
      List<Integer> couponAllocateIds =
          allocateList.stream().map(ClinicAllocateModel::getCouponAllocateId).collect(toList());
      ...
      errorBo =
          generateAllocateDetail(
              allocateList, couponId, submitDate, coupon.getCouponCode(), generateDate);
      if (errorBo.getError() != null) {
        return ResponseUtil.error(errorBo.getError());
      }
      long end = System.currentTimeMillis();
      log.info(
          "卡券[{}]生成分配完成，执行时间[{}]秒[{}]毫秒", couponId, (end - start) / 1000, (end - start) % 1000);
  ...
  public RestErrorBo generateAllocateDetail(
      List<ClinicAllocateModel> allocateList,
      Integer couponId,
    ...
    // 1. 计算每个组织卡券信息
    List<Future<AllocateNumBo>> boFutureList =
        calculateNumber(allocateList, sumGenerateNum, boLatch);
    boLatch.await();
    // 组织优惠券结果转Bo
    ...
    // 2. 计算每个卡券的生成信息
    List<Future<Card>> cardFutureList =
        createCardEntity(
            couponId, couponCode, numBoList, loginUserId, generateDate, cardLatch, sumAllocate);
    cardLatch.await();
    // 取出卡券明细任务的执行结果
              ...
              .map(
                  obj -> {
                    ...
                    OrganizationInfo orgInfo =
                        systemServiceFeign.findOrgInfoByOrgId(obj.getAllocateOrgId());
  ...
  public List<ExportCardAllocateVo> getExportCardAllocateList(GenerateAllocateCardQuery query) {
    ...
    if (CollectionUtils.isNotEmpty(allocateIds)) {
      ...
      List<Integer> orgIds =
          Lists.newArrayList(
              Sets.newHashSet(cardList.stream().map(Card::getOrgId).collect(toList())));
          ...
          cardList.stream()
              .map(
                  obj -> {
                    ...
                    vo.setCardPassword(
                        new String(Base64.getDecoder().decode(obj.getCardPassword())));
                    return vo;
                  })
              .collect(toList());
    }
  ...
  public PageInfo<CouponSalePageVo> getCouponSalePage(CouponSaleQuery query) {
    ...
    List<CouponSalePageVo> list =
        page.getResult().stream()
            .map(
                obj -> {
                  CouponSalePageVo vo =
                      BeanCopierUtils.generalCopyBean(obj, CouponSalePageVo.class);
  ...
  public PageInfo<CardSalePageVo> getCardSalePage(CardSaleQuery query) {
    ...
    mapper.listCardInfosByParam(
        query.getCardNumber(),
        query.getSoldTypeList(),
        query.getCardStatsList(),
        query.getPhoneNumber(),
        query.getCouponId(),
        query.getOrgId());
    // 实体转换为pageVo
    List<CardSalePageVo> list =
        page.getResult().stream().map(this::cardConvertPageVo).collect(toList());
      ...
      for (Integer cardId : cardIds) {
        ...
        if (orgAllocateBo == null) {
          ...
          return ResponseUtil.error(
              DiscountError.ORG_COUPON_NOT_ALLOCATE, orgName, couponInfo.getName());
        }
        // 检查该组织该优惠券售卖数量
  ...
  public CardQrCodeVo cardQrCodeCheck(Integer cardId) {
    ...
    if (ACTIVATED.equals(card.getStatus())
        || PARTIAL_USE.equals(card.getStatus())
        || USE_ALL.equals(card.getStatus())) {
  ...
  public ResponseResult<CardActiveDetailVo> getCardDetailByMachine(String qrCode) {
    ...
    List<String> data =
        Lists.newArrayList(Splitter.on(":").trimResults().omitEmptyStrings().split(qrCodeData));
    ...
    return ResponseUtil.success(
        mapper.findByCardNumAndPass(card.getCardNumber(), card.getCardPassword()));
  }
  ...
  public ResponseResult configSharer(Integer patientId, Integer cardId, ConfigSharerForm form) {
    ...
    List<String> shareIds =
        Lists.newArrayList(
            Splitter.on(",").trimResults().omitEmptyStrings().split(form.getSharerIdStr()));
  ...
  public List<PatientCardSharerVo> getConfiguredSharer(Integer cardId) {
    ...
    if (StringUtils.isNotBlank(card.getSharer())) {
      ...
      List<Integer> sharerIds =
          Arrays.stream(card.getSharer().split(",")).map(Integer::parseInt).collect(toList());
  ...
  public PageInfo<PatientCardBaseVo> getPatientCardPage(Integer patientId, PatientCardQuery query) {
    ...
    mapper.listPatientCardsByParam(
        patientId, query.getCouponName(), query.getCouponTypeList(), query.getQueryType());
    // 对象转换
    List<PatientCardBaseVo> list =
        page.getResult().stream()
            .map(obj -> this.patientCardBoConvertVo(query.getQueryType(), obj))
            .collect(toList());
  ...
  public ResponseResult<List<OrderItemUseBo>> choiceBenefitBo(PatientChooseBenefitForm form) {
    ...
    if (CollectionUtils.isEmpty(benefitVo.getDiscountVoList())
        && CollectionUtils.isEmpty(benefitVo.getMemberCardVoList())
        && CollectionUtils.isEmpty(benefitVo.getExchangeVoList())
        && CollectionUtils.isEmpty(benefitVo.getPackageVoList())
    ...
    try {
      ...
      errorBo =
          needLockKeys(
              assembleCardIds(form),
              patientId,
              RedisConstants.LOCK_CHOICE_CARD,
              DiscountError.CARD_HAS_CHOICE);
      if (errorBo.getError() != null) {
        return ResponseUtil.error(errorBo.getError(), errorBo.getMsg());
      }
      // 获取订单明细
      List<OrderDetail> orderDetails =
          treatmentServiceFeign.findOrderDetailByOrderRecordId(orderId);
  ...
  private void calculateBenefit(
      PatientChooseBenefitForm form,
      PatientOptionalBenefitVo benefitVo,
      List<OrderItemUseBo> orderItemBos) {
    try {
      Integer orgId = form.getOrgId();
   ...
   * @param orgId 组织
   */
  private void singleItemUseBenefit(
      BenefitUseBo benefitUseBo, OrderItemUseBo orderItem, Integer orgId) {
    // 兑换券项目优惠
    ...
    // 折扣券券项目优惠
    if (orderItem.getBenefitAmount() == null
        && memberBenefitBo == null
        && discountBenefitBos != null) {
      setUpSingleBenefitInfoForOrder(
          orderItem, orgId, Collections.singletonList(discountBenefitBos));
    }
    // 会员卡优惠
   ...
   * @param i 项目的哪一次数量
   */
  private void multiItemUseBenefit(
      BenefitUseBo benefitUseBo, OrderItemUseBo orderItem, Integer orgId, Integer i) {
    int mark = 0;
    // 兑换券项目优惠
    List<PatientUseBenefitBo> exchangeBenefitBos = benefitUseBo.getExchangeBenefitBos();
    // 兑换券项目优惠
  ...
  private PatientOrderBenefitVo transformBenefitInfo(List<OrderItemUseBo> orderItemBos) {
    ...
    if (CollectionUtils.isNotEmpty(orderItemBos)) {
      ...
      BigDecimal totalBenefitAmount =
          orderItemBos.stream()
              .filter(obj -> obj.getBenefitAmount() != null)
              .map(OrderItemUseBo::getBenefitAmount)
              .reduce(BigDecimal.ZERO, BigDecimal::add);
      result.setBenefitTotalAmount(totalBenefitAmount);
      List<PatientItemBenefitVo> itemList =
          orderItemBos.stream()
              .filter(obj -> CollectionUtils.isNotEmpty(obj.getItemUseBenefitBos()))
              .map(
                  obj -> {
                    BigDecimal benefitAmount =
                        obj.getBenefitAmount() == null ? BigDecimal.ZERO : obj.getBenefitAmount();
                    ...
                    List<ItemUseBenefitVo> itemUseBenefitVos =
                        BeanCopierUtils.listGeneralCopyBean(
                            obj.getItemUseBenefitBos(), ItemUseBenefitVo.class);
   ...
   */
  private void setUpSingleBenefitInfoForOrder(
      OrderItemUseBo orderItem, Integer orgId, List<PatientUseBenefitBo> benefitBos) {
    for (PatientUseBenefitBo benefitBo : benefitBos) {
      if (checkMixUsed(benefitBo)) {
        // 订单项目id对应的可用的优惠券信息
        ItemBenefitUseDetailBo benefitUseDetailBo =
            findBenefitForOrderItem(orgId, benefitBo, orderItem);
        ...
        if (MEMBER_CARD.equals(couponType) || benefitUseDetailBo != null) {
          ...
          BigDecimal oldBenefitAmount =
              orderItem.getBenefitAmount() == null ? BigDecimal.ZERO : orderItem.getBenefitAmount();
          // 订单项目应收金额（原价 - 已优惠金额）
          ...
          if (receivableAmount.compareTo(BigDecimal.valueOf(0)) > 0) {
            if (EXCHANGE.equals(couponType) || SPECIAL_PACKAGE.equals(couponType)) {
              ...
              benefitAmount =
                  receivableAmount.compareTo(packageUnitPrice) > 0
                      ? receivableAmount.subtract(packageUnitPrice)
                          ...
                          BigDecimal.valueOf(1)
                              .subtract(
                                  benefitBo
                                      .getDiscountRate()
                                      .divide(
                                          BigDecimal.valueOf(100), 4, BigDecimal.ROUND_HALF_UP)))
            ...
            if (MEMBER_CARD.equals(couponType)) {
              ...
              if (ZERO.equals(orderItem.getType())) {
                ClinicTariffMemberPrice tariff =
                    treatmentServiceFeign.findClinicTariffMemberPrice(
                        orgId, benefitBo.getCardId(), orderItem.getItemId());
                if (tariff == null) {
                  memberPrice =
              ...
              if (ONE.equals(orderItem.getType())) {
                ClinicOralTariffMemberPrice oral =
                    treatmentServiceFeign.findClinicOralTariffMemberPrice(
                        orgId, benefitBo.getCardId(), orderItem.getItemId());
                if (oral == null) {
                  memberPrice =
                      originalPrice.multiply(
              ...
              // 订单项目id对应的可用的优惠券信息
              benefitAmount =
                  receivableAmount.subtract(memberPrice).setScale(2, BigDecimal.ROUND_HALF_UP);
              buildOrderProperty(
                  benefitAmount,
                  orderItem,
                  benefitBo,
            ...
            if (VOUCHER.equals(couponType)) {
              if (benefitBo.getFace().compareTo(BigDecimal.valueOf(0)) > 0) {
                benefitAmount =
                    receivableAmount.compareTo(benefitBo.getFace()) >= 0
                        ? benefitBo.getFace()
                        : receivableAmount;
                // 个体项目设置优惠相关信息
                buildOrderProperty(
   ...
   */
  private void buildOrderProperty(
      BigDecimal benefitAmount,
      OrderItemUseBo order,
      PatientUseBenefitBo benefitBo,
      ItemBenefitUseDetailBo benefitUseDetailBo,
      Integer benefitType,
      Integer couponType,
      Integer itemIndex) {
    BigDecimal oldBenefitAmount = order.getBenefitAmount();
    order.setBenefitAmount(
        oldBenefitAmount == null ? benefitAmount : oldBenefitAmount.add(benefitAmount));
    BigDecimal receivableAmount = order.getReceivableAmount();
    order.setBenefitDiscountRate(
        order.getBenefitAmount().divide(receivableAmount, 4, RoundingMode.HALF_UP));
    ...
    if (CollectionUtils.isNotEmpty(itemUseBenefitBos)) {
      // 上一个id顺序
      preId =
          itemUseBenefitBos.stream()
              .max(Comparator.comparing(ItemUseBenefitBo::getId))
              .get()
              .getId();
    }
    // 优惠券使用顺序
  ...
  private int setUpMultiItemForOrder(
      List<PatientUseBenefitBo> benefitBos,
      OrderItemUseBo orderItem,
      Integer orgId,
      Integer itemIndex) {
    for (PatientUseBenefitBo benefitBo : benefitBos) {
      if (checkMixUsed(benefitBo)) {
        ...
        if (benefitUseDetailBo != null || MEMBER_CARD.equals(couponType)) {
          ...
          BigDecimal originalPrice =
              orderItem
                  .getReceivableAmount()
                  .divide(BigDecimal.valueOf(orderItem.getQuantity()), 4, BigDecimal.ROUND_HALF_UP);
          // 订单项目index已优惠金额
          ...
          if (receivableAmount.compareTo(BigDecimal.valueOf(0)) > 0) {
            if (EXCHANGE.equals(couponType) || SPECIAL_PACKAGE.equals(couponType)) {
              ...
              benefitAmount =
                  receivableAmount.compareTo(packageUnitPrice) > 0
                      ? benefitAmount = receivableAmount.subtract(packageUnitPrice)
                      : BigDecimal.valueOf(0);
              buildOrderProperty(
                  benefitAmount,
                  orderItem,
  ...
  private boolean checkMixUsed(PatientUseBenefitBo benefitBo) {
    ...
    // 当前个体被优惠的卡券共用属性
    Set<Integer> set =
        Sets.newHashSet(mixUsedMap.values().stream().flatMap(Collection::stream).collect(toSet()));
  ...
  private PatientOptionalBenefitVo getPatientBenefit(
      Integer patientId, Integer orderId, Integer orgId) {
    // 查询患者可用优惠
    ...
    for (PatientBenefitBo benefitBo : benefitBos) {
      if (VOUCHER.equals(benefitBo.getCouponType()) || DISCOUNT.equals(benefitBo.getCouponType())) {
        ...
        List<Integer> itemTypes =
            orderDetail.stream().map(obj -> obj.getType().intValue()).collect(toList());
        // 校验订单价目项目是否可用优惠并赋值
      ...
      if (SPECIAL_PACKAGE.equals(benefitBo.getCouponType())) {
        // 校验套餐券价目是否可用优惠并赋值
        if (checkPackageAndSetUsable(
            FALSE.getCode(), itemMap, benefitBo, SPECIAL_PACKAGE.getCode())) {
          continue;
        }
        // 校验套餐券商品是否可用优惠并赋值
    ...
    // 排序（截止时间 asc）
    Comparator<PatientBenefitBo> comparator =
        Comparator.comparing(PatientBenefitBo::getItemUsable, Comparator.reverseOrder())
            .thenComparing(PatientBenefitBo::getUseDeadline)
  ...
  private boolean checkPackageAndSetUsable(
      Integer itemType,
      Map<Integer, Set<Integer>> itemMap,
      PatientBenefitBo benefitBo,
      Integer couponType) {
    // 订单项目明细
    ...
    if (CollectionUtils.isNotEmpty(items) && CollectionUtils.isNotEmpty(orderItemIds)) {
      Set<Integer> itemIds =
          items.stream().map(SpecialPackageCouponItem::getItemId).collect(toSet());
      // 查询卡券使用数量信息
      List<CouponItemUseBo> list =
          mapper.getCouponItemUseInfo(
              benefitBo.getCouponId(), benefitBo.getCardId(), itemType, couponType);
      // 设置优惠券是否可以作用订单项目
  ...
  private boolean checkExchangeAndSetUsable(
      Integer itemType,
      Map<Integer, Set<Integer>> itemMap,
      PatientBenefitBo benefitBo,
      Integer couponType) {
  ...
  private boolean setItemUsableStatus(
      Set<Integer> itemIds,
    ...
    if (CollectionUtils.isNotEmpty(itemIds)) {
      ...
      if (CollectionUtils.isNotEmpty(joinIds)) {
        // 查找订单的项目是否可用该卡券
        Optional<CouponItemUseBo> optional =
            list.stream()
                .filter(obj -> joinIds.contains(obj.getItemId()) && TRUE.equals(obj.getUsable()))
                .findFirst();
  ...
  private boolean checkAndSetItemUsable(
      Integer itemType,
      List<Integer> itemTypes,
      PatientBenefitBo benefitBo,
      Map<Integer, Set<Integer>> itemMap) {
  ...
  private PatientOptionalBenefitVo benefitBoConvertVo(
      Integer patientId, List<PatientBenefitBo> benefitBos) {
    ...
    if (CollectionUtils.isNotEmpty(ownerIds)) {
      List<PatientBaseInfoVo> owners =
          patientFeign.findPatientInfoByIds(Lists.newArrayList(ownerIds));
    ...
    // bo结果集映射
    Map<Integer, List<PatientBenefitBo>> boMap =
        benefitBos.stream().collect(groupingBy(PatientBenefitBo::getCouponType));
    boMap.forEach(
        (k, v) -> {
          if (DISCOUNT.equals(k)) {
            List<PatientDiscountVo> discountVoList =
                v.stream()
                    .map(
                        bo -> {
                          PatientBaseInfoVo owner = finalPatientMap.get(bo.getOwnerId());
                          PatientDiscountVo discountVo =
                              BeanCopierUtils.generalCopyBean(bo, PatientDiscountVo.class);
          ...
          if (EXCHANGE.equals(k)) {
            List<PatientExchangeVo> exchangeVoList =
                v.stream()
                    .map(
                        bo -> {
                          PatientBaseInfoVo owner = finalPatientMap.get(bo.getOwnerId());
                          PatientExchangeVo exchangeVo =
                              BeanCopierUtils.generalCopyBean(bo, PatientExchangeVo.class);
                          if (!patientId.equals(bo.getOwnerId())) {
          ...
          if (SPECIAL_PACKAGE.equals(k)) {
            List<PatientPackageVo> packageVoList =
                v.stream()
                    .map(
                        bo -> {
                          PatientBaseInfoVo owner = finalPatientMap.get(bo.getOwnerId());
                          PatientPackageVo packageVo =
                              BeanCopierUtils.generalCopyBean(bo, PatientPackageVo.class);
                          if (!patientId.equals(bo.getOwnerId())) {
          ...
          if (VOUCHER.equals(k)) {
            List<PatientVoucherVo> voucherVoList =
                v.stream()
                    .map(
                        bo -> {
                          PatientBaseInfoVo owner = finalPatientMap.get(bo.getOwnerId());
                          PatientVoucherVo voucherVo =
                              BeanCopierUtils.generalCopyBean(bo, PatientVoucherVo.class);
                          if (!patientId.equals(bo.getOwnerId())) {
  ...
  private List<PatientMemberCardVo> getPatientMemberCards(Integer patientId) {
    ...
    if (memberInfo != null) {
            ...
            secondaryMemberInfoVos.stream()
                .map(
                    obj -> {
                      ...
                      memberCardVo.setMemberCardRate(
                          BigDecimal.valueOf(obj.getRate()).setScale(2, BigDecimal.ROUND_HALF_UP));
      ...
      if (masertMemberInfoVo != null) {
        ...
        memberCardVo.setMemberCardRate(
            BigDecimal.valueOf(masertMemberInfoVo.getRate()).setScale(2, BigDecimal.ROUND_HALF_UP));
  ...
  private CardQrCodeVo checkCouponDeadline(Integer couponId, Integer type) {
    ...
    vo.setActivationDeadline(
        deadline == null
            ? COUPON_ALWAYS_EFFECT
            : deadline
                .toInstant()
  ...
  private List<Future<Card>> createCardEntity(
      Integer couponId,
      String couponCode,
      List<AllocateNumBo> numBoList,
      Integer loginUserId,
      LocalDateTime generateDate,
      CountDownLatch cardLatch,
      int totalTask) {
        ...
        cardFutureList.add(
            cardThreadPool.submit(
                () -> {
                  ...
                  card.setCardNumber(
                      String.format(couponCode + "%06d", generateNum.getAndIncrement()));
                  // 生成卡密
  ...
  private List<AllocateNumBo> getCalculateBoFutureResult(List<Future<AllocateNumBo>> boFutureList)
      throws Exception {
  ...
  private List<Future<AllocateNumBo>> calculateNumber(
      List<ClinicAllocateModel> allocateList, int maxNum, CountDownLatch latch) {
    // 创建线程分配卡券
  ...
  private GenerateAllocatePageVo allocateBoConvertVo(GenerateAllocatePageBo bo) {
    ...
    // 提交人信息
    SysUserInfoDetail submitUser =
        systemServiceFeign.findSysUserEmployeeInfoByUserId(bo.getSubmitUserId());
    // 配给人信息
    if (bo.getAllocateUserId() != null) {
      SysUserInfoDetail allocateUser =
          systemServiceFeign.findSysUserEmployeeInfoByUserId(bo.getAllocateUserId());
  ...
  private RestErrorBo checkCouponAllocate(
      List<ClinicAllocateModel> allocateList, Integer couponId, LocalDateTime submitDate) {
    RestErrorBo errorBo = RestErrorBo.getInstance();
    // 该产品卡券已生成数量
    int sumGenerateNum = mapper.getSumNumByCouponId(couponId);
    // 所有组织卡券总数
    ...
    Example example = new Example(CouponAllocate.class);
    example.createCriteria().andEqualTo("couponId", couponId).andEqualTo("crtTime", submitDate);
    ...
    // 已分配优惠券映射
    Map<Integer, CouponAllocate> entityMap =
        list.stream().collect(toMap(CouponAllocate::getId, Function.identity()));
    for (ClinicAllocateModel allocateModel : allocateList) {
      ...
      // 校验组织分配优惠券数量和时间
      if (couponAllocate == null
          || !allocateModel.getAllocateNum().equals(couponAllocate.getAllocateNum())
  ...
  private void updateCardForSold(
      Card card, CardSoldForm form, Integer loginUserId, LocalDateTime now, Integer saleChannelId) {
                ...
                Joiner.on(":")
                    .join(
                        new BCryptPasswordEncoder(UserConstant.PW_ENCODER_SALT)
                            .encode(
                                Joiner.on(":").join(card.getCardNumber(), card.getCardPassword())),
                        card.getId())
                    .getBytes()));
   ...
   */
  private void updateOwnActiveCard(
      Integer patientId, OwnCardActiveForm form, Integer loginUserId, Integer couponId) {
    Integer activeOrgId =
        StringUtils.isBlank(BaseContextHandler.getOrgId())
            ? null
   ...
   */
  private Card insertOtherActiveCard(
      Integer patientId, OtherCardActiveForm form, Integer loginUserId) {
    Integer activeOrgId =
        StringUtils.isBlank(BaseContextHandler.getOrgId())
            ? null
            : Integer.valueOf(BaseContextHandler.getOrgId());
  ...
  private RestErrorBo checkCardForOwnActive(Integer payId, Card card) {
    ...
    if (card.getActiveOrgId() != null
        || ACTIVATED.equals(card.getStatus())
        || card.getPatientId() != null
        || PARTIAL_USE.equals(card.getStatus())
        || USE_ALL.equals(card.getStatus())) {
  ...
  private PatientCardBaseVo patientCardBoConvertVo(Integer queryType, PatientCardBo bo) {
    ...
    if (TRUE.equals(queryType)) {
      PatientShareCardVo shareCardVo =
          BeanCopierUtils.generalCopyBean(bo, PatientShareCardVo.class);
      // 查询卡主
  ...
  private Map<Integer, Set<Integer>> getVoucherItemInfo(Integer couponId, Integer itemType) {
    List<VoucherDiscountItem> voucherDiscountItems =
        getCouponItemByCouponId(couponId, voucherDiscountItemMapper, VoucherDiscountItem.class);
    // <项目类型，选择范围，项目明细集合>
    Map<Integer, Map<Integer, Set<Integer>>> voucherDiscountItemMap =
        voucherDiscountItems.stream()
            .collect(
                groupingBy(
                    VoucherDiscountItem::getType,
                    groupingBy(
                        obj -> obj.getChoiceRangType().intValue(),
                        mapping(VoucherDiscountItem::getItemId, toSet()))));
    // 代金折扣券基础，价目范围映射 <选择范围，项目id（分类/明细）>
  ...
  private List<PackageCouponItem> getExchangeItemInfo(Integer couponId, Integer itemType) {
    List<PackageCouponItem> packageCouponItems =
        getCouponItemByCouponId(couponId, packageCouponItemMapper, PackageCouponItem.class);
    // <项目类型，项目明细集合>
    Map<Integer, List<PackageCouponItem>> exchangeItemMap =
        packageCouponItems.stream().collect(groupingBy(PackageCouponItem::getType));
  ...
  private List<SpecialPackageCouponItem> getPackageItemInfo(Integer couponId, Integer itemType) {
    ...
    example.createCriteria().andEqualTo("couponId", couponId);
    List<SpecialPackageCouponItem> specialPackageCouponItems =
        specialPackageCouponItemMapper.selectByExample(example);
    // <项目类型，项目明细集合>
    Map<Integer, List<SpecialPackageCouponItem>> specialPackageItemMap =
        specialPackageCouponItems.stream().collect(groupingBy(SpecialPackageCouponItem::getType));
   ...
   * @return errorbo
   */
  private RestErrorBo checkChoiceBenefitForItem(
      PatientOptionalBenefitVo benefitVo, PatientChooseBenefitForm form) {
    RestErrorBo errorBo = RestErrorBo.getInstance();
    ...
    if (useMemberCardId != null) {
      if (CollectionUtils.isEmpty(memberCardVoList)) {
        ...
        isExist =
            memberCardVoList.stream()
                .map(PatientMemberCardVo::getMemberCardId)
                .collect(toList())
                .contains(useMemberCardId);
        if (!isExist) {
      ...
      if (CollectionUtils.isEmpty(discountVoList)) {
        ...
        isExist =
            discountVoList.stream()
                .filter(obj -> TRUE.equals(obj.getItemUsable()))
                .map(PatientDiscountVo::getCardId)
                .collect(toList())
                .contains(useDiscountId);
        if (!isExist) {
    ...
    if (CollectionUtils.isNotEmpty(useExchangeIds)) {
      if (CollectionUtils.isEmpty(exchangeVoList)) {
        ...
        // 可作用在项目的兑换券卡券
        List<Integer> exchangeIdsForItem =
            exchangeVoList.stream()
                .filter(obj -> TRUE.equals(obj.getItemUsable()))
                .map(PatientExchangeVo::getCardId)
                .collect(toList());
        for (Integer exchangeId : useExchangeIds) {
          if (!exchangeIdsForItem.contains(exchangeId)) {
            ...
            errorBo.setMsg(
                card == null
                    ? null
                    : card.getOrgId() == 0 ? card.getThirdCardNumber() : card.getCardNumber());
            return errorBo;
          }
        }
        Map<Integer, Long> limitCountMap =
            exchangeVoList.stream()
                .filter(obj -> useExchangeIds.contains(obj.getCardId()))
    ...
    if (CollectionUtils.isNotEmpty(usePackageIds)) {
      if (CollectionUtils.isEmpty(packageVoList)) {
        ...
        // 可作用在项目的兑换券卡券
        List<Integer> packageIdsForItem =
            packageVoList.stream()
                .filter(obj -> TRUE.equals(obj.getItemUsable()))
                .map(PatientPackageVo::getCardId)
                .collect(toList());
        ...
        Map<Integer, Long> limitCountMap =
            packageVoList.stream()
                .filter(obj -> choiceCardIds.contains(obj.getCardId()))
                .collect(groupingBy(PatientPackageVo::getCouponId, counting()));
        // 校验单个账单使用限制数
        errorBo =
            checkLimitCount(limitCountMap, SpecialPackageCoupon.class, SPECIAL_PACKAGE.getCode());
        if (errorBo.getError() != null) {
          return errorBo;
        }
      }
    ...
    if (CollectionUtils.isNotEmpty(useVoucherIds)) {
      if (CollectionUtils.isEmpty(voucherVoList)) {
        ...
        List<Integer> choiceCardIds = Lists.newArrayList();
        // 可作用在项目的兑换券卡券
        List<Integer> voucherIdsForItem =
            voucherVoList.stream()
                .filter(obj -> TRUE.equals(obj.getItemUsable()))
                .map(PatientVoucherVo::getCardId)
                .collect(toList());
   ...
   * @return 错误bo
   */
  private RestErrorBo checkLimitCount(
      Map<Integer, Long> limitCountMap, Class<?> clazz, Integer couponType) {
    RestErrorBo errorBo = RestErrorBo.getInstance();
  ...
  private void assignedCouponItemDetail(BenefitUseBo benefitUseBo) {
    ...
    List<PatientUseBenefitBo> voucherBenefitBos = benefitUseBo.getVoucherBenefitBos();
    // 兑换券设置可使用项目
    exchangeBenefitBos.forEach(
        obj -> {
          List<CouponItemUseBo> couponItemUseBos =
              mapper.getCouponItemUseInfo(
                  obj.getCouponId(), obj.getCardId(), null, EXCHANGE.getCode());
          obj.setExchangeCouponItemDetail(couponItemUseBos);
        });
    // 套餐券设置可使用项目
    packageBenefitBos.forEach(
        obj -> {
          List<CouponItemUseBo> couponItemUseBos =
              mapper.getCouponItemUseInfo(
                  obj.getCouponId(), obj.getCardId(), null, SPECIAL_PACKAGE.getCode());
          obj.setPackageCouponItemDetail(couponItemUseBos);
        });
    // 代金折扣券基础，价目范围映射 <选择范围，项目id（分类/明细）>
    if (discountBenefitBo != null) {
      List<VoucherDiscountItem> discountItems =
          getCouponItemByCouponId(
              discountBenefitBo.getCouponId(),
              voucherDiscountItemMapper,
              VoucherDiscountItem.class);
      if (CollectionUtils.isNotEmpty(discountItems)) {
        assignedCouponItem(discountItems, discountBenefitBo);
      }
    }
    // 代金券设置可使用项目
    voucherBenefitBos.forEach(
        voucherBo -> {
          // 代金折扣券基础，价目范围映射 <选择范围，项目id（分类/明细）>
          List<VoucherDiscountItem> voucherItems =
              getCouponItemByCouponId(
                  voucherBo.getCouponId(), voucherDiscountItemMapper, VoucherDiscountItem.class);
   ...
   */
  private void assignedCouponItem(
      List<VoucherDiscountItem> discountItems, PatientUseBenefitBo disCountVoucherItemBo) {
    Set<Integer> set = Sets.newHashSet();
    // 价目全选
    Optional<VoucherDiscountItem> tariffOptional =
        discountItems.stream()
            .filter(
                obj ->
                    FALSE.equals(obj.getType())
                        && SELECT_ALL.equals(obj.getChoiceRangType().intValue()))
            .findAny();
    if (tariffOptional.isPresent()) {
      disCountVoucherItemBo.setDiscountVoucherCouponItemDetail(
          null, FALSE.getCode(), SELECT_ALL.getCode());
    }
    // 商品全选
    Optional<VoucherDiscountItem> shopOptional =
        discountItems.stream()
            .filter(
                obj ->
                    TRUE.equals(obj.getType())
                        && SELECT_ALL.equals(obj.getChoiceRangType().intValue()))
            .findAny();

    if (shopOptional.isPresent()) {
      disCountVoucherItemBo.setDiscountVoucherCouponItemDetail(
          null, TRUE.getCode(), SELECT_ALL.getCode());
    }
    // 选择价目分类
    List<Integer> tariffCategoryIds =
        discountItems.stream()
            .filter(
                obj ->
                    FALSE.equals(obj.getType())
                        && SELECT_ITEM_CATEGORY.equals(obj.getChoiceRangType().intValue()))
            .map(VoucherDiscountItem::getItemId)
            .collect(toList());
    // 选择商品分类
    ...
    // 设置折扣券的价目项目
    disCountVoucherItemBo.setDiscountVoucherCouponItemDetail(
        set, FALSE.getCode(), SELECT_ITEM_CATEGORY.getCode());
    set.clear();
    ...
    // 设置折扣券的商品项目
    disCountVoucherItemBo.setDiscountVoucherCouponItemDetail(
        set, TRUE.getCode(), SELECT_ITEM_CATEGORY.getCode());

    // 设置价目明细
    Set<Integer> tariffItemIds =
        discountItems.stream()
            .filter(
                obj ->
                    FALSE.equals(obj.getType())
                        && SELECT_ITEM_DETAIL.equals(obj.getChoiceRangType().intValue()))
            .map(VoucherDiscountItem::getItemId)
            .collect(toSet());
    disCountVoucherItemBo.setDiscountVoucherCouponItemDetail(
        tariffItemIds, FALSE.getCode(), SELECT_ITEM_DETAIL.getCode());
    // 设置商品明细
    Set<Integer> shopItemIds =
        discountItems.stream()
            .filter(
                obj ->
                    TRUE.equals(obj.getType())
            ...
            .collect(toSet());
    disCountVoucherItemBo.setDiscountVoucherCouponItemDetail(
        shopItemIds, TRUE.getCode(), SELECT_ITEM_DETAIL.getCode());
  }
   ...
   */
  private ItemBenefitUseDetailBo findBenefitForOrderItem(
      Integer orgId, PatientUseBenefitBo useBenefitBo, OrderItemUseBo order) {
    ...
    if (CollectionUtils.isNotEmpty(usableClinic) && usableClinic.contains(orgId)) {
      ...
      if (EXCHANGE.equals(couponType) || SPECIAL_PACKAGE.equals(couponType)) {
        ...
        Optional<ItemBenefitUseDetailBo> optional =
            benefitUseDetail.stream()
                .filter(
                    obj ->
                        obj.getItemId().equals(order.getItemId())
                            && obj.getType().equals(order.getType())
                            && obj.getCount() > 0)
                .findFirst();
        if (optional.isPresent()) {
          return optional.get();
      ...
      if (DISCOUNT.equals(couponType) || VOUCHER.equals(couponType)) {
        ...
        Optional<ItemBenefitUseDetailBo> optional =
            benefitUseDetail.stream()
                .filter(
                    obj ->
                        (SELECT_ALL.equals(obj.getItemId())
                                || obj.getItemId().equals(order.getItemId()))
                            && obj.getType().equals(order.getType()))
                .findFirst();
        if (optional.isPresent()) {
   ...
   * @param item 订单项目
   */
  private List<PatientUseBenefitBo> getBenefitBosByItem(
      List<PatientUseBenefitBo> useBenefitBos, OrderItemUseBo item) {
    Integer type = item.getType();
    Integer itemId = item.getItemId();
    return useBenefitBos.stream()
        .filter(
            obj -> {
              ...
              Optional<ItemBenefitUseDetailBo> optional =
                  detail.stream()
                      .filter(
                          itemDetail ->
                              type.equals(itemDetail.getType())
                                  && itemId.equals(itemDetail.getItemId()))
                      .findAny();
              if (optional.isPresent()) {
                return true;
              }
              Optional<ItemBenefitUseDetailBo> voucherOptional =
                  detail.stream()
                      .filter(
                          voucherItem ->
                              (SELECT_ALL.equals(voucherItem.getItemId())
                                      || voucherItem.getItemId().equals(itemId))
                                  && voucherItem.getType().equals(type))
                      .findFirst();
  ...
  private Map<Integer, UseClinicBo> getCouponUseClinic(List<Integer> couponIds) {
    if (CollectionUtils.isNotEmpty(couponIds)) {
      ...
      return useClinicIdStr.stream()
          .collect(toMap(UseClinicBo::getCouponId, Function.identity(), (v1, v2) -> v2));
    }
    return Collections.emptyMap();
  }

  public void assignedBenefitBos(
      BenefitUseBo benefitUseBo,
      PatientOptionalBenefitVo benefitVo,
      PatientChooseBenefitForm form) {
    List<PatientExchangeVo> exchangeVoList = benefitVo.getExchangeVoList();
    List<Integer> exchangeIds = form.getExchangeIds();
    ...
    if (CollectionUtils.isNotEmpty(exchangeIds) && CollectionUtils.isNotEmpty(exchangeVoList)) {
      Map<Integer, PatientExchangeVo> exchangeVoMap =
          exchangeVoList.stream().collect(toMap(PatientExchangeVo::getCardId, Function.identity()));
      exchangeVoList =
          exchangeIds.stream()
              .filter(cardId -> exchangeVoMap.get(cardId) != null)
              .map(exchangeVoMap::get)
              .collect(toList());
      // 优惠券vo 生成 对应bo对象
      List<PatientUseBenefitBo> exchangeUseBos =
          exchangeVoList.stream()
              .map(
                  obj -> {
                    PatientUseBenefitBo exchangeBenefitBo =
                        BeanCopierUtils.generalCopyBean(obj, PatientUseBenefitBo.class);
                    exchangeBenefitBo.setCouponType(EXCHANGE.getCode());
                    exchangeBenefitBo.setLimitCount(
                        getCouponLimitCount(
                            obj.getCouponId(), PackageCoupon.class, EXCHANGE.getCode()));
    ...
    if (CollectionUtils.isNotEmpty(packageIds) && CollectionUtils.isNotEmpty(packageVoList)) {
      Map<Integer, PatientPackageVo> packageVoMap =
          packageVoList.stream().collect(toMap(PatientPackageVo::getCardId, Function.identity()));
      List<PatientUseBenefitBo> packageUseBos =
          packageIds.stream()
              .filter(cardId -> packageVoMap.get(cardId) != null)
              .map(
                  cardId -> {
                    PatientPackageVo packageVo = packageVoMap.get(cardId);
                    PatientUseBenefitBo packageBenefitBo =
                        BeanCopierUtils.generalCopyBean(packageVo, PatientUseBenefitBo.class);
                    packageBenefitBo.setCouponType(SPECIAL_PACKAGE.getCode());
                    packageBenefitBo.setLimitCount(
                        getCouponLimitCount(
                            packageVo.getCouponId(),
                            SpecialPackageCoupon.class,
                            SPECIAL_PACKAGE.getCode()));
    ...
    if (discountId != null && CollectionUtils.isNotEmpty(discountVoList)) {
      List<PatientUseBenefitBo> discountUseBos =
          discountVoList.stream()
              .filter(obj -> discountId.equals(obj.getCardId()))
              .map(
                  obj -> {
                    PatientUseBenefitBo discountBenefitBo =
                        BeanCopierUtils.generalCopyBean(obj, PatientUseBenefitBo.class);
                    ...
                    discountBenefitBo.setDiscountRate(
                        discountMapper.selectOne(discountCoupon).getDiscountRate());
    ...
    if (memberCardId != null && CollectionUtils.isNotEmpty(memberCardVoList)) {
      List<PatientUseBenefitBo> memberUseBos =
          memberCardVoList.stream()
              .filter(obj -> memberCardId.equals(obj.getMemberCardId()))
              .map(
                  obj -> {
                    PatientUseBenefitBo memberCard =
                        BeanCopierUtils.generalCopyBean(obj, PatientUseBenefitBo.class);
                    // 会员卡类型id
    ...
    if (CollectionUtils.isNotEmpty(voucherIds) && CollectionUtils.isNotEmpty(voucherVoList)) {
      Map<Integer, PatientVoucherVo> voucherVoMap =
          voucherVoList.stream().collect(toMap(PatientVoucherVo::getCardId, Function.identity()));
      List<PatientUseBenefitBo> voucherUseBos =
          voucherIds.stream()
              .filter(cardId -> voucherVoMap.get(cardId) != null)
              .map(
                  cardId -> {
                    PatientVoucherVo voucherVo = voucherVoMap.get(cardId);
                    PatientUseBenefitBo voucherBenefitBo =
                        BeanCopierUtils.generalCopyBean(voucherVo, PatientUseBenefitBo.class);
                    voucherBenefitBo.setCouponType(VOUCHER.getCode());
                    voucherBenefitBo.setLimitCount(
                        getCouponLimitCount(
                            voucherVo.getCouponId(), VoucheCoupon.class, VOUCHER.getCode()));
  ...
  private void assignedUseClinicForBo(List<PatientUseBenefitBo> benefitUseBo) {
    // 查询优惠券的可用门诊
    Map<Integer, UseClinicBo> useClinicMap =
        getCouponUseClinic(
            benefitUseBo.stream().map(PatientUseBenefitBo::getCouponId).collect(toList()));
    if (!org.springframework.util.CollectionUtils.isEmpty(useClinicMap)) {
      // 设置优惠券可用门诊
      benefitUseBo.stream()
          .filter(obj -> useClinicMap.get(obj.getCouponId()) != null)
          .forEach(
              obj -> {
                String useClinicIdStr = useClinicMap.get(obj.getCouponId()).getUseClinicIdStr();
                if (StringUtils.isNotBlank(useClinicIdStr)) {
                  obj.setUsableClinic(
                      Lists.newArrayList(Splitter.on(",").split(useClinicIdStr)).stream()
                          .map(Integer::valueOf)
                          .collect(toList()));
                }
              });
  ...
  private RestErrorBo needLockKeys(
      List<Integer> resourceIds, Integer requestId, String lockPrefix, RestError error) {
    RestErrorBo errorBo = RestErrorBo.getInstance();
  ...
  private Set<String> getExistLock(Integer requestId, String lockPrefix) {
    ...
    if (CollectionUtils.isNotEmpty(keys)) {
      return keys.stream()
          .filter(key -> requestId.equals(Integer.valueOf(redisUtils.get(key))))
  ...
  public ResponseResult<Boolean> manualLock(List<Integer> ids, String lockPrefix) {
    ...
    if (CollectionUtils.isNotEmpty(ids)) {
      Integer loginUserId = Integer.valueOf(BaseContextHandler.getUserID());
      RestErrorBo errorBo =
          needLockKeys(ids, loginUserId, lockPrefix, DiscountError.CARD_IS_ON_SALE);
      if (errorBo.getError() != null) {
        return ResponseUtil.error(errorBo.getError(), errorBo.getMsg());
      }
    }
  ...
  private int countByBenefit(Integer orderId) {
    ...
    example.createCriteria().andEqualTo("orderId", orderId).andEqualTo("deleted", FALSE.getCode());
  ...
  private OrderItemChangeBo getItemBenefitByIndex(OrderItemUseBo orderItem, Integer index) {
    ...
    Optional<OrderItemChangeBo> first =
        changeBos.stream().filter(obj -> index.equals(obj.getIndex())).findFirst();
