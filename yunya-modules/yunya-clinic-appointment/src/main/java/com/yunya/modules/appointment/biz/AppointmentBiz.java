...
public class AppointmentBiz extends BaseBiz<AppointmentMapper, Appointment> {
  ...
  /** 预约操作记录服务 */
  @Autowired private AppointmentOperateRecordBiz appointOperateRecordBiz;

  /** 时长分解服务 */
  @Autowired private AppointmentSplitBiz appointmentSplitBiz;

  /** 预约修改服务 */
  @Autowired private AppointmentModifyRecordBiz appointmentModifyRecordBiz;

  /** 按照患者数量排序 */
  private final String ORDER_PATIENTNUM = "patientNum";
  /** 按照日期排序 */
  private final String ORDER_DATE = "date";
  /** 升序 */
    ...
    if (null == appointConflictResult) {
      ...
      if (index <= 0) {
        throw new ClientServiceException(
            "【" + patientame + "】预约失败！", OperationCodeConstants.OBJECT_EDIT_FAIL);
      }
      ...
      if (form.getSplitList() != null || !form.getSplitList().isEmpty()) {
        ...
        if (splitResult == null || splitResult <= 0) {
          throw new ClientServiceException(
              (String) "分解时长失败！", OperationCodeConstants.OBJECT_EDIT_FAIL);
        }
      }
      ...
      Integer appointmentOperateRecord =
          appointOperateRecordBiz.insertAppointmentOperateRecord(operationModel);
      if (appointmentOperateRecord <= 0) {
        throw new ClientServiceException(
            "【" + patientame + "】的预约操作记录添加失败！", OperationCodeConstants.OBJECT_EDIT_FAIL);
      }
      // 如果添加预约成功，则返回预约成功信息
  ...
  public Appointment updateAppointStatus(Integer id, Byte appointState, String remarks) {
    ...
    if (appointment.getAppointStatus() != 0) {
      throw new ClientServiceException(
          "【预约未到】以外的情况不允许编辑预约！", OperationCodeConstants.OBJECT_EDIT_FAIL);
    }
  ...
  public ResponseResult continueUpdateAppointment(AppointmentBaseForm appointmentForm) {
    ...
    // 保存预约更新被修改的日期、医生
    appointmentModifyRecordBiz.saveAppointModify(
        mapper.selectByPrimaryKey(appointmentForm.getId()), appointmentForm);
  ...
  public List<AppointmentListItemVo> findAppointmentListByExample(AppointListQuery query) {
    ...
    // 设置预约医生/助手信息
    appointmentVos.forEach(
        appointmentVo -> {
          AppointmentListItemVo build =
              EntityUtils.build(appointmentVo, AppointmentListItemVo.class);
          // 查询预约医生信息
          SysUserInfoDetail dentistInfoDetail =
              remoteSystemServiceFeign.findSysUserEmployeeInfoByUserId(build.getDentistId());
          build.setDentistName(dentistInfoDetail.getName());
          // 查询预约助手信息
          SysUserInfoDetail assistantInfoDetail =
              remoteSystemServiceFeign.findSysUserEmployeeInfoByUserId(build.getAssistantId());
          build.setAssistantName(assistantInfoDetail.getName());

          // 设置患者详细信息
          PatientTotalInfoVo patientInfo =
              patientCentralServiceFeign.findPatientTotalInfo(build.getPatientId());
          ...
          DepartmentRoom departmentRoom =
              remoteSystemServiceFeign.findDepartmentRoomById(appointmentVo.getClinicDeptRoomId());
          ...
          appointmentList.stream()
              .filter(
                  appointmentListItemVo ->
                      appointmentListItemVo.getAppointType().equals(query.getAppointType()))
    ...
    if (!StringHelper.isEmpty(query.getMedicalNumber())) {
      ...
      collect =
          collect.stream()
              .filter(
                  appointmentListItemVo ->
                      appointmentListItemVo.getMedicalNumber().equals(query.getMedicalNumber()))
              .collect(Collectors.toList());
    }
    ...
    if (!StringHelper.isEmpty(query.getDentistName())) {
      ...
      collect =
          collect.stream()
              .filter(
                  appointmentListItemVo ->
                      appointmentListItemVo.getDentistName().equals(query.getDentistName()))
              .collect(Collectors.toList());
    }
    ...
    if (!StringHelper.isEmpty(query.getSeach())) {
      ...
      collect =
          collect.stream()
              .filter(
                  appointmentListItemVo ->
                      appointmentListItemVo.getPatientName().contains(query.getSeach())
  ...
  /**
   * 根据条件查询预约可视图（医生维度）按预约患者数量降序排列 可用范围 根据医生id、排班时间查询医生预约信息
   *
   * @param query 查询条件
   * @return
   */
  public List<AppointmentDimensionVo> findAppointmentPatientDimensionByExample(
      AppointmentPatientDimensionByDayQuery query) {
    ...
    // 查询当前天有排班的员工列表
    EmployeeScheduleResultVO scheduleResultVO =
        employeeAttendServiceFeign.findList(employeeScheduleQueryForm);
    ...
    shiftWorkDatas.forEach(
        userWorkVO -> {
          ...
          AppointmentDimensionVo appointmentDimensionVo =
              mapper.findAppointmentDimensionInfoByDateAndDentistId(
                  query.getStartDate(), query.getEndDate(), userId, query.getOrgId());
          ...
          // 组合患者基本信息
          List<AppointmentPatientCardVo> appointmentPatientCardVos =
              appointmentDimensionVo.getAppointmentPatientCardVos();
          appointmentPatientCardVos.forEach(
              appointmentPatientCardVo -> {
                Integer patientId = appointmentPatientCardVo.getPatientId();
                PatientBaseInfo patientInfo =
                    patientCentralServiceFeign.findPatientInfoById(patientId);
    ...
    if (ORDER_PATIENTNUM.equals(query.getOrder()) && ORDER_BY_ASC.equals(query.getOrderBy())) {
      List<AppointmentDimensionVo> appointmentDimensionVosOrderByAsc =
          appointmentDimensionVoList.stream()
              .sorted(Comparator.comparing(AppointmentDimensionVo::getPatientNum))
              .collect(Collectors.toList());
      return appointmentDimensionVosOrderByAsc;

    } else if (ORDER_PATIENTNUM.equals(query.getOrder())
        && ORDER_BY_DESC.equals(query.getOrderBy())) {
      // 按患者预约数量降序排列
      List<AppointmentDimensionVo> appointmentDimensionVosOrderByDesc =
          appointmentDimensionVoList.stream()
              .sorted(Comparator.comparing(AppointmentDimensionVo::getPatientNum).reversed())
              .collect(Collectors.toList());
      ...
      List<AppointmentDimensionVo> appointmentDimensionVosOrderByDesc =
          appointmentDimensionVoList.stream()
              .sorted(Comparator.comparing(AppointmentDimensionVo::getCurrentDate).reversed())
              .collect(Collectors.toList());
      return appointmentDimensionVosOrderByDesc;
    }
    // 没有排序直接返回
  ...
  /**
   * 预约可视图列表（医生维度） 通过日期、门诊id、查询医生预约情况
   *
   * @param query
   * @return
   */
  public List<AppointmentDentistDimensionVo> findAppointmentDentistDimensionByExample(
      AppointmentPatientDimensionByDayQuery query) {
    ...
    List<AppointmentDimensionVo> appointmentDimensionVos =
        this.findAppointmentPatientDimensionByExample(query);

    SimpleDateFormat dateFormat = new SimpleDateFormat("yyyy-MM-dd");

    // 根据大医生id查询相关助手信息并且设置助手信息
    appointmentDimensionVos.forEach(
        appointmentDimensionVo -> {
          // 患者信息列表
          List<AppointmentDimensionVo> assistantInfoList = new ArrayList<>();
          AppointmentDimensionVo assistantSplitVo =
              EntityUtils.build(appointmentDimensionVo, AppointmentDimensionVo.class);
          AppointmentSplitQuery splitQuery = new AppointmentSplitQuery();

          assistantSplitVo
              .getAppointmentPatientCardVos()
              .forEach(
                  appointmentPatientCardVo -> {
                    // 设置预约id
                    ...
                    // 查询本次预约相关的分解助手信息
                    List<AppointmentSplitVo> appointSplitVo =
                        appointmentSplitBiz.findAppointmentSplitByExample(splitQuery);
                    ...
                    for (int index = 0; index < appointSplitVo.size(); index++) {
                      ...
                      // 通过feign查询助手详细信息
                      SysUserInfoDetail assistantDetailInfo =
                          remoteSystemServiceFeign.findSysUserEmployeeInfoByUserId(
                              appointmentSplitVo.getAssistantId());
          ...
          // 对助手信息进行排序   按患者数量排序
          List<AppointmentDimensionVo> sortByDescList =
              assistantInfoList.stream()
                  .sorted(Comparator.comparing(AppointmentDimensionVo::getPatientNum).reversed())
                  .collect(Collectors.toList());

          AppointmentDentistDimensionVo appointDentistDimensionVo =
              new AppointmentDentistDimensionVo();
          // 将大医生信息设置到医生维度信息模板中
    ...
    // 按患者数量对医生降序排序
    Collections.sort(
        appointmentDentistDimensionVoList,
        new Comparator<AppointmentDentistDimensionVo>() {
          @Override
          public int compare(AppointmentDentistDimensionVo o1, AppointmentDentistDimensionVo o2) {
            if (o1.getAppointmentDentistInfoVo().getPatientNum()
                > o2.getAppointmentDentistInfoVo().getPatientNum()) {
              return -1;
            }
            if (o1.getAppointmentDentistInfoVo().getPatientNum()
                < o2.getAppointmentDentistInfoVo().getPatientNum()) {
  ...
  private Map<String, Object> checkScheduling(AppointmentBaseModel appointmentBaseModel) {
    ...
    if (dentistId != null) {
      ...
      // 获取排班列表
      EmployeeScheduleResultVO employeeScheduleResult =
          employeeAttendServiceFeign.findList(employeeScheduleQueryForm);
      // 预约医生没有排班，返回空
   ...
   */
  private Map<String, Object> checkConflict(AppointmentBaseModel appointmentForm)
      throws ParseException {
    ...
    if (patientId != null) {
      List<AppointConflictInfoVo> patientList =
          mapper.findAppointListByPatientIdAndAppointStartTimeAndAppointEndTime(
              patientId, appointStartTime, appointEndTime);
      if (!StringHelper.isEmpty(patientList)) {
        patientList.forEach(
            appointConflictInfoVo -> {
              OrganizationInfo organizationInfo =
                  remoteSystemServiceFeign.findOrgInfoByOrgId(appointConflictInfoVo.getOrgId());
    ...
    if (dentistId != null) {
      List<AppointConflictInfoVo> dentisList =
          mapper.findAppointListByDentistIdAndAppointStartTimeAndAppointEndTime(
              dentistId, appointStartTime, appointEndTime);
      if (!StringHelper.isEmpty(dentisList)) {
        dentisList.forEach(
            appointConflictInfoVo -> {
              OrganizationInfo organizatioinInfo =
                  remoteSystemServiceFeign.findOrgInfoByOrgId(appointConflictInfoVo.getOrgId());
    ...
    if (deviceId != null) {
      List<AppointConflictInfoVo> deviceList =
          mapper.findAppointListByDeviceIdAndAppointStartTimeAndAppointEndTime(
  ...
  private Appointment transferFormToEntity(Object form) {
    if (!(form instanceof AppointmentBaseModel) && !(form instanceof AppointmentBaseForm)) {
      throw new ClientServiceException(
          "对象转换实体异常！", OperationCodeConstants.DATA_TRANSFORMATION_EXIST);
    }
    ...
    try {
      ...
      throw new ClientServiceException(
          "[时间格式转换异常]：" + e.getMessage(), OperationCodeConstants.DATA_TRANSFORMATION_EXIST);
    }
    ...
    PatientBaseInfo patientBaseInfo =
        patientCentralServiceFeign.findPatientInfoById(appointment.getPatientId());
  ...
  private Map<String, Object> editCheckConflict(Integer id, AppointmentBaseForm appointmentForm) {
    ...
    try {
      ...
      throw new ClientServiceException(
          "时间格式转换错误！", OperationCodeConstants.DATA_TRANSFORMATION_EXIST);
    }
    // 预约时长
    Integer time = appointmentForm.getAppointDuration();
    // 获取预约开始时间的毫秒值
    ...
    if (patientId != null) {
      ...
      List<AppointConflictInfoVo> appointConflictInfoVos =
          mapper.editCheckPatientConflict(
              id, appointmentForm.getPatientId(), appointStartTime, appointEndTime);
    ...
    if (dentistId != null) {
      List<AppointConflictInfoVo> appointConflictInfoVos =
          mapper.editCheckDentistConflict(
              id, appointmentForm.getPatientId(), appointStartTime, appointEndTime);
      if (!appointConflictInfoVos.isEmpty()) {
        // 存在医生预约冲突
        responseMapResult.put("status", 5);
