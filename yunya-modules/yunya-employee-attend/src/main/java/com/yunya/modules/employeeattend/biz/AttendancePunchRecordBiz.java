...
public class AttendancePunchRecordBiz extends BaseBiz<AttendancePunchRecordMapper, AttendancePunchRecord> {
    ...
    public void punch(AttendancePunchRecordForm attendancePunchRecordForm) {
        ...
        if (onPunchRecord==null || offPunchRecord == null) {
            throw new ClientServiceException("打卡失败，请刷新页面", OperationCodeConstants.PARAMETERS_IS_ILLEGAL);
        }
        if (onDutyIspunch.equals(AttendanceIsPunchEnum.UNPUNCH.getCode()) && offPunchRecord.getPunchType().equals(AttendanceTypeEnum.OFFDUTY.getCode())) {
        ...
        Byte source = offPunchRecord.getSource();
        if (!AttendanceSourceEnum.FIELD.getCode().equals(source)) {
            Integer addressId = checkPosition(attendancePunchRecordForm, offPunchRecord.getOrgId());
...
//        checkPosition(attendancePunchRecordForm,dbPunchRecord);
        ...
        Byte punchType = offPunchRecord.getPunchType();
        ...
        if (!punchStatus.equals(AttendanceStatusEnum.INVALID_PUNCH.getCode())) {
            ...
            if (punchType.equals(AttendanceTypeEnum.ONDUTY.getCode())) {//上班卡
                if (time.after(offPunchRecord.getStartTime())) {
                    punchStatus = AttendanceStatusEnum.LATER_PUNCH.getCode();//迟到打卡
                } else {
                    punchStatus = AttendanceStatusEnum.ONDUTY_PUNCH.getCode();//上班打卡
                }
            } else {//下班卡
                if (time.before(offPunchRecord.getEndTime())) {
                    punchStatus = AttendanceStatusEnum.EARLY_PUNCH.getCode();//早退打卡
                } else {
                    punchStatus = AttendanceStatusEnum.OFFDUTY_PUNCH.getCode();//下班打卡
                }
        ...
        Byte oldPunchStatus = offPunchRecord.getPunchStatus();
    ...
    public PageInfo<AttendanceStatisticsVO> statisticsPunchRecord(AttendanceStatisticsQueryForm queryForm) {
        Integer employeeId = queryForm.getUserId();
        ...
        if (employeeId != null) {
            recordQueryForm.setUserId(employeeId);
        }
        //打卡记录
        ...
        if (employeeId != null) {
            employeeScheduleQueryForm.setUserId(employeeId);
        }
        List<EmployeeScheduleVO> employeeScheduleVOS = employeeScheduleBiz.findEmployeeScheduleList(employeeScheduleQueryForm);
        ...
        for (EmployeeScheduleVO employeeScheduleVO : employeeScheduleVOS) {
            ...
            if (list!=null && !list.isEmpty()) {
                ...
                if (list.size() == 2) {
                    ...
                    if (AttendanceIsPunchEnum.UNPUNCH.getCode().equals(first.getIsPunch())) {
                        incrNum(unpunchNumMap,userId,orgId);
                    }
                    if (AttendanceIsPunchEnum.UNPUNCH.getCode().equals(last.getIsPunch())) {
                        incrNum(unpunchNumMap,userId,orgId);
                    }
                } else {
                    Date sTime = first.getStartTime();
...
//    }
    ...
    public PageInfo<AttendanceWorkDateMinuteVO> statisticsWorkDateByMinute(AttendanceStatisticsQueryForm queryForm) {
        ...
        // 打卡
        Map<Date, List<AttendancePunchRecordVO>> punchRecordMap = getPunchRecordMapGroupByDate(userId, null, betweenDate, andDate);
        // 手动补入时长
                ...
                if (tag == "rest") {
                    ...
                    if (tag == "work") {
                        ...
                        if (isPunch.equals(AttendanceIsPunchEnum.UNPUNCH.getCode())) {//未打卡
                            ...
                            if (notCurDate && orgId.equals(onPunchItem.getOrgId())
                                    && onPunchItem.getIsInScope().equals(AttendanceIsInScopeEnum.BELONG.getCode())) {
                                startTime = onPunchItem.getStartTime();
                                if (onPunchTime.after(startTime)) {
                                    startTime = onPunchTime;
                                }
                                endTime = onPunchItem.getEndTime();
                                if (offPunchTime.before(endTime)) {
                ...
                if (tag == "rest") {
                    ...
                    if (tag == "work") {
                        ...
                        if (isPunch.equals(AttendanceIsPunchEnum.UNPUNCH.getCode())) {//未打卡
                            ...
                            if (notCurDate && orgId.equals(offPunchItem.getOrgId())
                                    && offPunchItem.getIsInScope().equals(AttendanceIsInScopeEnum.BELONG.getCode())) {
            ...
            if (punchRecords!=null && !punchRecords.isEmpty()) {
                ...
                for (AttendancePunchRecordVO punchRecord : punchRecords) {
                    if (punchRecord.getIsPunch().equals(AttendanceIsPunchEnum.PUNCHED.getCode())) {
                        Date sTime = onStartTime = punchRecord.getStartTime();
                        Date eTime = offEndTime = punchRecord.getEndTime();
