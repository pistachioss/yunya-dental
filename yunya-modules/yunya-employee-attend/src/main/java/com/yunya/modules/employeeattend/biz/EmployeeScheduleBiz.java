...
public class EmployeeScheduleBiz extends BaseBiz<EmployeeScheduleMapper, EmployeeSchedule> {
  ...
  public  List<EmployeeScheduleExportVO> copy(EmployeeScheduleCopyForm employeeScheduleCopyForm){
    ...
    Date targetEndDate = calendar.getTime();//目标时段的结束时间

    List<EmployeeScheduleCopyVO> CopyEmployeeSchedules = mapper.selectAllByDate(targetStartDate, targetEndDate, employeeIdList);//目标时段内的排班列表

    Map<Integer, List<EmployeeScheduleCopyVO>> EmployeeScheduleMap =
            EmployeeSchedules.stream().collect((Collectors.groupingBy(EmployeeScheduleCopyVO -> EmployeeScheduleCopyVO.getEmployeeId())));//根据员工ID对复制时段内的排班列表进行分组
    Map<Integer, List<EmployeeScheduleCopyVO>> CopyEmployeeSchedulesMap =
            CopyEmployeeSchedules.stream().collect((Collectors.groupingBy(EmployeeScheduleCopyVO -> EmployeeScheduleCopyVO.getEmployeeId())));//根据员工ID对目标时段内的排班列表进行分组

    for (List<EmployeeScheduleCopyVO> forEmplist : EmployeeScheduleMap.values()) {//遍历根据ID分组后的map
      List<EmployeeScheduleCopyVO> copyList = CopyEmployeeSchedulesMap.get(forEmplist.get(0).getEmployeeId());//取出与当前遍历的员工id相同的目标排班的集合
      for (EmployeeScheduleCopyVO employeeScheduleCopyVO : forEmplist) {//遍历复制排班的 当前遍历的员工ID下的排班信息
        if (copyList != null) {//目标排班时间段不为空
          for (EmployeeScheduleCopyVO copyVO : copyList) {//遍历当前员工ID的目标排班 进行比对

            Calendar calendarCopy = Calendar.getInstance();//设置复制排班与目标排班的日期对应
            calendarCopy.setTime(employeeScheduleCopyVO.getWorkDate());
            calendarCopy.add(Calendar.DATE, days1);

            if (calendarCopy.getTime().getTime() == copyVO.getWorkDate().getTime()) {//日期相同的排班取出 比对时间段
              if (employeeScheduleCopyVO.getSecondEndTime() == null) {//判断是否启用第二个时间段
                employeeScheduleCopyVO.setSecondEndTime(employeeScheduleCopyVO.getFirstEndTime());
              }
              if (copyVO.getSecondEndTime() == null) {//判断是否启用第二个时间段
                copyVO.setSecondEndTime(copyVO.getFirstEndTime());
              }
              SimpleDateFormat dateFormat = new SimpleDateFormat("HH:mm:ss");
              Date copyVoEndTime = null;//获取时间段的时分秒格式进行比较
