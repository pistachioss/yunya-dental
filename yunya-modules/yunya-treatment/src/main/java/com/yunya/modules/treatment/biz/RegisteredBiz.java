...
public class RegisteredBiz extends BaseBiz<RegisteredMapper, Registered> {
  ...
  public Integer save(RegisteredModel model) {
    ...
    if (null != appointmentId) {
      ...
      Integer appointmentOrgId = Integer.valueOf(BaseContextHandler.getOrgId());
      if (null != appointment) {
        appointment.setAppointStatus((byte) 1);
        appointmentFeign.updateAppointment(appointment);
        appointmentOrgId = appointment.getOrgId();
      }
  ...
  public List<ReferredRrportVO> referredReport(ReferredRrportForm referredRrportForm) {
    ...
    //        List<Integer>userList = reList.stream().map(p ->
    // p.getUserId()).collect(Collectors.toList());
    //        List<Integer>refList = reList.stream().map(p ->
  ...
  private void setPatientInfo(
    ...
    // 患者信息列表
    List<PatientTotalInfoVo> patientInfoByIds =
        remotePatientCentralServiceFeign.findPatientTotalInfo(patientIds);
    // 欠费金额列表
    ...
    appointDentistIds.stream()
        .sequential()
        .collect(Collectors.toCollection(() -> appointAssistentIds));
    // 根据预约医生ID和预约助手ID列表
    List<SysUserInfoDetail> appointDentistAndAssistentInfoList =
        systemServiceFeign.findSysUserEmployeeInfoByUserIds(appointAssistentIds);
    // 根据预约科室ID集合查询预约科室信息列表
    List<DepartmentRoom> appointDepartmentRooms =
        systemServiceFeign.findDepartmentRoomByIds(appointDeptRoomIds);
    // 根据挂号医生ID集合和挂号助手ID集合查询挂号医生、助手信息列表
    // 将dentistIds合并到assistentIds中
    registeredDentistIds.stream()
        .sequential()
        .collect(Collectors.toCollection(() -> registeredAssistentIds));
    List<SysUserInfoDetail> registeredDentistAndAssistentInfoList =
        systemServiceFeign.findSysUserEmployeeInfoByUserIds(registeredAssistentIds);
    // 根据挂号科室ID集合查询挂号科室信息列表
    List<DepartmentRoom> registeredDepartmentRooms =
        systemServiceFeign.findDepartmentRoomByIds(registeredDeptRoomIds);
    ...
    vos.forEach(
        patientEntity -> {
          ...
          if (StringHelper.isNotEmpty(patientInfoByIds)) {
            List<PatientTotalInfoVo> collect =
                patientInfoByIds.stream()
                    .filter(patientTotalInfoVo -> patientTotalInfoVo.getId().equals(patientId1))
            ...
            if (StringHelper.isNotEmpty(collect)) {
              ...
              patientEntity.setMedicalNumber(
              ...
              if (StringHelper.isNotEmpty(memberTypeByIds)) {
                List<MemberType> collect1 =
                    memberTypeByIds.stream()
                        .filter(
                            memberType ->
                                memberType.getId().equals(patientTotalInfoVo.getMemberTypeId()))
                        .collect(Collectors.toList());
                if (StringHelper.isNotEmpty(collect1)) {
                  MemberType memberType = collect1.get(0);
                  patientEntity.setMemberIcon(memberType.getIcon());
                  patientEntity.setMemberCardName(patientTotalInfoVo.getInservice()?memberType.getName():memberType.getOldName());
                }
              }
            }
          }
          ...
          // 设置候诊患者挂号信息
          setRegisteredInfo(
              patientEntity, registeredDentistAndAssistentInfoList, registeredDepartmentRooms);
          // 欠费金额
          if (StringHelper.isNotEmpty(debtAmountList1)) {
            List<DebtAmountModel> collect =
                debtAmountList1.stream()
                    .filter(debtAmountModel -> debtAmountModel.getPatientId().equals(patientId1))
                    .collect(Collectors.toList());
            if (StringHelper.isNotEmpty(collect)) {
    ...
    if (StringHelper.isNotEmpty(appointmentList)) {
      ...
      List<Appointment> collect =
          appointmentList.stream()
              .filter(appointment -> appointment.getId().equals(appointId))
              .collect(Collectors.toList());
      if (StringHelper.isNotEmpty(collect)) {
        ...
        List<SysUserInfoDetail> sysUserInfoDetailList =
            dentistAndAssistentInfoList.stream()
        ...
        List<SysUserInfoDetail> assistantInfos =
            dentistAndAssistentInfoList.stream()
                .filter(sysUserInfoDetail -> sysUserInfoDetail.getUserId().equals(assistantId))
                .collect(Collectors.toList());
        ...
        List<DepartmentRoom> departmentRoomInfos =
            departmentRooms.stream()
                .filter(departmentRoom -> departmentRoom.getId().equals(deptRoomId))
                .collect(Collectors.toList());
  ...
  private void setRegisteredInfo(
    ...
    if (StringHelper.isNotEmpty(registeredDentistAndAssistentInfoList)) {
      ...
      List<SysUserInfoDetail> collect =
          registeredDentistAndAssistentInfoList.stream()
      ...
      List<SysUserInfoDetail> regAssistantInfos =
          registeredDentistAndAssistentInfoList.stream()
              .filter(sysUserInfoDetail -> sysUserInfoDetail.getUserId().equals(regAssistantId))
              .collect(Collectors.toList());
    ...
    if (StringHelper.isNotEmpty(registeredDepartmentRooms)) {
      ...
      List<DepartmentRoom> collect =
          registeredDepartmentRooms.stream()
  ...
  public RegisteredVO registeredInfoDetail(Integer id) {
    ...
    if (registeredVO != null) {
      ...
      if (deptRoomId != null) {
        DepartmentRoom departmentRoomById =
            this.systemServiceFeign.findDepartmentRoomById(deptRoomId);
  ...
  public List<RegisteredVO> registeredInfoDetails(List<Integer> ids) {
    ...
    List<RegisteredVO> vos = mapper.registeredInfoDetails(ids);
    ...
    // 注入医生，助手，科室信息
    for (RegisteredVO registeredVO : vos) {
      // 设置组织信息
      registeredVO.setOrgId(registeredVO.getOrgId());
      // 注入医生信息
      if (StringHelper.isNotEmpty(dentistInfoList)) {
        Integer dentistId = registeredVO.getDentistId();
        assert dentistInfoList != null;
        List<SysUserInfoDetail> dentistInfos =
            dentistInfoList.stream()
                .filter(sysUserInfoDetail -> dentistId.equals(sysUserInfoDetail.getUserId()))
                .collect(Collectors.toList());
      ...
      if (StringHelper.isNotEmpty(assistantInfoList)) {
        Integer assistantId = registeredVO.getAssistantId();
        if (assistantId != null) {
          assert assistantInfoList != null;
          List<SysUserInfoDetail> assistantInfos =
              assistantInfoList.stream()
                  .filter(sysUserInfoDetail -> assistantId.equals(sysUserInfoDetail.getUserId()))
                  .collect(Collectors.toList());
          if (StringHelper.isNotEmpty(assistantInfos)) {
      ...
      if (StringHelper.isNotEmpty(deptRoomInfoList)) {
        Integer deptRoomId = registeredVO.getDeptRoomId();
        if (deptRoomId != null) {
          assert deptRoomInfoList != null;
          List<DepartmentRoom> departmentRooms =
              deptRoomInfoList.stream()
                  .filter(departmentRoom -> deptRoomId.equals(departmentRoom.getId()))
                  .collect(Collectors.toList());
      ...
      if (StringHelper.isNotEmpty(patientTotalInfoList)) {
        Integer patientId = registeredVO.getPatientId();
        assert patientTotalInfoList != null;
        List<PatientTotalInfoVo> patientTotalInfoVos =
            patientTotalInfoList.stream()
