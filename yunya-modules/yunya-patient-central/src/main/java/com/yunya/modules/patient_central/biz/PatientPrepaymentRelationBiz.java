...
public class PatientPrepaymentRelationBiz
  ...
  public ResponseResult addPrepaymentLink(PatientPrepaymentRelationModel model) {
    ...
    // 发送预付款关联消息
    // remoteRabbitMqServiceFeign.sendMessage(patientPrepaymentRelationyi.getId(), 1, 0,
  ...
  public ResponseResult recharge(PrepaidRechargeModel model) {
    if (model.getRechargeCardNumber() == null) {
      ...
      if (model
              ...
              .compareTo(model.getPrepaidRechargeTollRecordModel().getCreditAmount())
          != 0) {
        return ResponseUtil.fail(
            OperationCodeConstants.PARAMETERS_IS_ILLEGAL, "充值金额与入账金额不相等!", null);
      }
    }
    PatientPrepaymentsInfo patientPrepaymentsInfo = patientMemberInfoBiz.openIfAbsent(model);
    // 查询预付款余额 增加余额
    if (patientPrepaymentsInfo != null) {
      ...
      patientPrepaymentsInfo.setUptId(optId);
      patientPrepaymentsInfo.setUpdName(optName);
      patientPrepaymentsInfo.setUpdTime(now);
      ...
      prepaidRechargeRecord.setCrtTime(now);
      ...
      prepaidRechargeRecord.setUptId(optId);
      prepaidRechargeRecord.setUpdName(optName);
      prepaidRechargeRecord.setUpdTime(now);
      ...
      prepaidRechargeTollRecord.setOrgId(orgId);
      prepaidRechargeTollRecord.setCrtId(optId);
      prepaidRechargeTollRecord.setCrtName(optName);
      prepaidRechargeTollRecord.setCrtTime(now);
      prepaidRechargeTollRecord.setUptId(optId);
      prepaidRechargeTollRecord.setUpdName(optName);
      prepaidRechargeTollRecord.setUpdTime(now);
        ...
        if (patientPrepaymentsInfo.getPatientId() != null && model.getCardId() != null) {
          ...
          ResponseResult result =
              remoteDiscountFeign.ownActiveCard(
                  patientPrepaymentsInfo.getPatientId(), ownCardActiveForm);
  ...
  public void memberSendMessages(Object object, Integer type) {
    ...
    if (type == 0) {
      eventCode = SmsAutosendEventEnum.PREPAY_CHARGE.getCode();
    ...
    if (patientBaseInfo != null) {
      ...
      if (type == 0) {
        ...
        // 预付款卡号
        templateParam.put(
            SmsTemplateItemEnum.PREPAID_ACCOUNT.getAction(), prepaidRechargeRecord.getPrepaidId());
        // 预付款充值金额
        ...
        templateParam.put(
            SmsTemplateItemEnum.PREPAID_RECHARGE_AMOUNT.getAction(),
        ...
        templateParam.put(
            SmsTemplateItemEnum.PREPAID_REMAINING_AMOUNT.getAction(),
            prepaidRechargeRecord
        ...
        // 预付款卡号
        templateParam.put(
            SmsTemplateItemEnum.PREPAID_ACCOUNT.getAction(), prepaidExpendRecord.getPrepaidId());
        // 预付款消费金额
        ...
        templateParam.put(
            SmsTemplateItemEnum.PREPAID_CONSUMPTION_AMOUNT.getAction(),
            prepaidExpendRecord
                .getExpendPrincipal()
                .add(expendGift)
                .setScale(2, BigDecimal.ROUND_HALF_UP));
      ...
      redisUtils.lPush(SMS_SEND_MESSAGE_QUEUE + orgId, smsModel);
    }
  }
    ...
    if (!StringHelper.isEmpty(resultList)) {
      for (PrepaidRechargeRecordVo prepaidRechargeRecordVo : resultList) {
        ...
        OrganizationInfo organizationInfo =
            remoteSystemServiceFeign.findOrgInfoByOrgId(prepaidRechargeRecordVo.getOrgId());
      ...
      if (patientPrepaymentsInfo != null) {
        ...
        patientPrepaymentsInfo.setPrepaymentPrincipal(
            patientPrepaymentsInfo
                .getPrepaymentPrincipal()
                .add(prepaidExpend.getExpendPrincipal()));
        patientPrepaymentsInfo.setPrepaymentBonus(
        ...
        return ResponseUtil.fail(
            OperationCodeConstants.RETURN_MOBILE_ISNULL, "未查询到预付款", patientPrepaymentsInfo);
      }
    }
    return ResponseUtil.fail(
        OperationCodeConstants.RETURN_MOBILE_ISNULL, "未查询到预付款消费记录", prepaidExpend);
  }
   ...
   */
  public Integer updPatientPrepaidInfo(PrepaidRevocationFeeModel model) {
    PrepaidExpendRecord prepaidExpendRecord = new PrepaidExpendRecord();
    prepaidExpendRecord.setPrepaidId(model.getPrepaidCard());
    prepaidExpendRecord.setBillPayRecordId(model.getBillPayRecordId());
    PrepaidExpendRecord prepaidExpendRecordvo =
        prepaidExpendRecordMapper.selectOne(prepaidExpendRecord);
    if (prepaidExpendRecordvo != null) {
      prepaidExpendRecordvo.setInservice(false);
      prepaidExpendRecordMapper.updateByPrimaryKeySelective(prepaidExpendRecordvo);
