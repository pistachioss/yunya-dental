...
public class StatementPatientCardRefundDetailVO implements Serializable {
  ...
  /** 会员卡或预付卡余额（含赠金） */
  @Excel(name = "会员卡或预付卡余额（含赠金）", scale = 2, cellType = Excel.ColumnType.NUMERIC)
  @ApiModelProperty("剩余余额（含赠金）")
  private BigDecimal restTotalAmount;
  /** 充值本金金额 */
  @Excel(name = "退本金金额", scale = 2, cellType = Excel.ColumnType.NUMERIC)
  @ApiModelProperty("退本金金额")
  private BigDecimal refundPrincipleAmount;
  /** 充值赠金金额 */
  @Excel(name = "退赠送金额", scale = 2, cellType = Excel.ColumnType.NUMERIC)
