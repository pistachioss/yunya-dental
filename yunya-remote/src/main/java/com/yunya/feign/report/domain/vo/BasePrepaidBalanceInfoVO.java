 ...
 * @description: 预付款余额结存数据模型
 * @since: 1.0.0
 */
@Data
@ToString
@ApiModel("预付款余额结存数据模型")
public class BasePrepaidBalanceInfoVO implements Serializable {
    ...
    @ApiModelProperty("本期转入本金（预付款）")
    private BigDecimal thisTransferPrincipalInPrepaid;

    /** 本期转入赠金（预付款） */
    @Excel(name = "本期转入赠金（预付款）", cellType = Excel.ColumnType.NUMERIC, isStatistics = true, type = EXPORT)
    @ApiModelProperty("本期转入赠金（预付款）")
    private BigDecimal thisTransferBonusInPrepaid;

    /** 本期转出本金（预付款） */
    @Excel(name = "本期转出本金（预付款）", cellType = Excel.ColumnType.NUMERIC, isStatistics = true, type = EXPORT)
    @ApiModelProperty(value = "本期转出本金（预付款）")
    private BigDecimal thisTransferPrincipalOutPrepaid;

    /** 本期转出赠金（预付款） */
    @Excel(name = "本期转出赠金（预付款）", cellType = Excel.ColumnType.NUMERIC, isStatistics = true, type = EXPORT)
    @ApiModelProperty(value = "本期转出赠金（预付款）")
    private BigDecimal thisTransferBonusOutPrepaid;

    /** 本期转出本金（会员卡） */
    @Excel(name = "本期转出本金（会员卡）", cellType = Excel.ColumnType.NUMERIC, isStatistics = true, type = EXPORT)
    @ApiModelProperty("本期转出本金（会员卡）")
    private BigDecimal thisTransferPrincipalOutMember;

    /** 本期转出赠金（会员卡） */
    @Excel(name = "本期转出赠金（会员卡）", cellType = Excel.ColumnType.NUMERIC, isStatistics = true, type = EXPORT)
    @ApiModelProperty("本期转出赠金（会员卡）")
    private BigDecimal thisTransferBonusOutMember;
}